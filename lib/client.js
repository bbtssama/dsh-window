...
// In-place TABLE CELL editing. The cell keeps its box (the hidden mirror holds the shape, so the
// column width does not jump), and the input draws the characters being edited — a transparent input
// hid everything typed, which is why the first attempt looked like it did nothing.
'.dn-td,.dn-th{position:relative;}',
'.dn-cell-edit{outline:2px solid rgba(79,124,255,.6);outline-offset:-2px;border-radius:3px;background:rgba(79,124,255,.06);}',
'.dn-cell-edit .dn-cell-mirror{visibility:hidden;}',
'.dn-cell-in{position:absolute;left:0;top:0;width:100%;height:100%;margin:0;padding:inherit;border:0;box-sizing:border-box;background:transparent;color:inherit;caret-color:var(--dsw-alias-label-primary,#1b1b1b);outline:none;font:inherit;line-height:inherit;white-space:pre;overflow:hidden;text-align:inherit;}',
...
// A task box is a BUTTON: reset the UA face so it keeps the bullet's look, and make it read as
// something clickable (hover + focus ring) without moving the line.
'.dn-task{background:none;border:0;padding:0;margin:0;font:inherit;color:inherit;line-height:inherit;cursor:pointer;border-radius:3px;}',
'.dn-task:hover{background:rgba(79,124,255,.14);}',
'.dn-task:focus-visible{outline:2px solid rgba(79,124,255,.6);outline-offset:1px;}',
...
const assetCache = {}
// A failure is NOT remembered forever: the same relative path can be missing from one note and
// present in another, and a transient failure (the host answering while it is still starting up, a
// note not yet active) used to poison the path for the whole life of the page — every image then
// showed 「图片无法读取」 until the reader reloaded. Failures are remembered briefly (so a genuinely
// broken path does not hammer the host) and then retried.
const assetFail = {}
const ASSET_RETRY_MS = 5000
/**
 * The cache key. A relative reference like `images/x.png` means a DIFFERENT file in every note — each
 * note has its own asset root — so the note has to be part of the key. Keyed by the href alone, a
 * miss in one note (or another note's bytes) was served for it in the next one.
 */
function assetCacheKey(note, href) { return String(note || '') + '\u0000' + String(href || '') }
/** Drop every cached image: the mirror may have changed under us (sync / reload from disk). */
function clearAssetCache() {
  for (const k in assetCache) delete assetCache[k]
  for (const k in assetFail) delete assetFail[k]
}
...
/* TASK-PURE-START */
/**
 * The source line of a task item with its box toggled — `- [ ]` ⇄ `- [x]`. Only the box changes:
 * indentation, the bullet character and the rest of the line are left exactly as they were, and a
 * line that is not a task item at all returns null so a stray click changes nothing.
 */
function toggleTaskLine(line) {
  const src = String(line === undefined || line === null ? '' : line)
  const m = /^(\s*[-*+]\s+\[)([ xX])(\])/.exec(src)
  if (!m) return null
  return m[1] + (m[2] === ' ' ? 'x' : ' ') + m[3] + src.slice(m[0].length)
}
/** The same for one 1-based line of a whole document: null when that line is not a task item. */
function toggleTaskText(text, lineNo) {
  const src = String(text === undefined || text === null ? '' : text)
  const all = src.split(String.fromCharCode(10))
  const at = Math.round(Number(lineNo)) - 1
  if (!(at >= 0 && at < all.length)) return null
  const next = toggleTaskLine(all[at])
...
function parseBlocks(text) {
  ...
  while (i < ls.length) {
    ...
    if (line.indexOf('|') >= 0 && i + 1 < ls.length && isSep(ls[i + 1])) {
      ...
      // The header row IS a row — rendered as <th>, and a real SOURCE line (marks and the cell editor
      // need its line number). It used to be skipped entirely, so every table lost its header and the
      // first DATA row was promoted to the header slot (a recorded table showed 标记-清除 where the
      // header 算法 belonged). A header line with no text at all (`|  |  |`, what Typora leaves behind
      // when a table is inserted and its header is never filled in) is skipped as before, and in that
      // case the first data row keeps the header slot, so nothing changes for those notes.
      const headRaw = ls[i]
      const headHasText = /[^\s|:-]/.test(headRaw)
      if (headHasText) rows.push({ raw: headRaw, line: i + 1, head: true })
      let first = true
      i += 2
      while (i < ls.length && ls[i].indexOf('|') >= 0 && ls[i].trim() !== '') {
        rows.push({ raw: ls[i], line: i + 1, head: headHasText ? false : first })
        first = false
        i++
      }
...
function InlineEditor(props) {
  ...
  React.useEffect(function () {
    // A new session (another block / another note) starts from the CURRENT buffer, not from whatever
    // this instance happened to hold: the mirror is built imperatively, so a reused instance would
    // otherwise show stale text or an empty box.
    lastRef.current = String(propsRef.current.value === undefined || propsRef.current.value === null ? '' : propsRef.current.value)
    fillAll()
    const ta = taRef.current
    if (ta) {
      try {
    ...
    // Re-fill when the edit SESSION changes (another block, another note): the mirror's children are
    // built imperatively, so a reused instance would otherwise show an empty box. Typing never
    // changes the session, so this cannot fight the reader.
  }, [props.session])
  const mirror = isCode
...
ctx.effect(function () { return function () { try { if (ownStyle) ownStyle.remove() } catch (err) { } } })
    ...
    function NoteImage(props) {
      ...
      // Why it failed, for the tooltip only: the visible label stays the short one.
      const [failReason, setFailReason] = React.useState('')
      React.useEffect(function () {
        if (!props.href || !isLocalRef(props.href)) return undefined
        const key = assetCacheKey(props.note, props.href)
        if (assetCache[key] !== undefined) {
          setDataUrl(assetCache[key]); setState('ok'); setFailed(false)
          return undefined
        }
        const failedAt = assetFail[key] || 0
        if (failedAt > 0 && Date.now() - failedAt < ASSET_RETRY_MS) { setFailed(true); return undefined }
        let alive = true
        // The path is re-resolved against the note that is on screen NOW (props.note), and a failure
        // only silences this path for ASSET_RETRY_MS — a transient failure must not leave the image
        // broken for the rest of the page's life.
        host.call('asset', { path: props.href }).then(function (r) {
          if (r && r.ok && r.dataUrl) {
            delete assetFail[key]
            assetCache[key] = r.dataUrl
            if (alive) { setDataUrl(r.dataUrl); setState('ok'); setFailed(false) }
          } else {
            assetFail[key] = Date.now()
            if (alive) { setFailed(true); setFailReason(String((r && r.error) || '读取失败')) }
          }
        }).catch(function (err) {
          assetFail[key] = Date.now()
          if (alive) { setFailed(true); setFailReason(String((err && err.message) || err)) }
        })
        return function () { alive = false }
      }, [props.href, props.note])
      ...
      // The reason rides the tooltip: the visible label stays short, but hovering tells the reader (and
      // whoever is debugging) exactly what the host answered.
      const failProps = failReason ? { title: failReason } : {}
      if (failed) return h('span', Object.assign({ className: 'dn-imgfail' }, failProps), ['\ud83d\uddbc 图片无法读取：' + String(props.href || '')])
      if (state === 'ok' && src) {
        return h('img', {
          ...
          onError: function () { setFailed(true); setFailReason('图片解码失败（data URL 有效但浏览器读不出来）'); bumpGeometry() },
        })
      }
    ...
    function NoteCard(props) {
      ...
      // The table cell being edited in place: { note, line, at, len, ci, value, wasText }. One cell at a
      // time — a table row IS one source line, so the commit rewrites that cell's characters inside it.
      const [cellEdit, setCellEdit] = React.useState(null)
      const cellEditRef = React.useRef(null)
      ...
      function navRecordJump(nav, incoming, destLine, leftLine) {
        ...
        const list = nav.list.slice(0, nav.at + 1)
        // A measurement of 0 (the card is collapsed, the DOM is not up yet) must never ERASE the line
        // this entry had recorded: keep it, and let the next real measurement replace it.
        const left = Math.round(Number(leftLine)) || 0
        list[nav.at] = { name: nav.list[nav.at].name, line: left >= 1 ? left : (Math.round(Number(nav.list[nav.at].line)) || 0) }
      ...
      /**
       * The stack the HOST has for this session (it rides every state answer). Returns null when there
       * is none, and drops a record past the ttl — the same 24h rule as the browser copy, applied to
       * `updatedAt` instead of a local timestamp.
       */
      function navFromHost(raw, now, ttl) {
        if (!raw || typeof raw !== 'object' || !Array.isArray(raw.list) || raw.list.length === 0) return null
        const list = []
        ...
        const ts = Math.round(Number(raw.updatedAt)) || 0
        if (Number(now) - ts > ttl) return null
        const at = Math.min(Math.max(0, Math.round(Number(raw.at)) || 0), list.length - 1)
      ...
      // Set once if the HOST refuses the persisted stack (an older host without saveNav): said out
      // loud a single time instead of every save.
      const navWarnRef = React.useRef(false)
      React.useEffect(function () {
      ...
      function saveNavNow() {
        ...
        writeNavStore(store)
        // And to the HOST, which keeps it in the session's own file: the browser copy is only a fast
      ...
      if (sidRef.current !== lastSidRef.current) {
        ...
        // -1 = "the next nonce from this session is a baseline, not news" (see applyState).
        seenJumpRef.current = -1
      }
      ...
      function applyState(r) {
        ...
        // ever looked at that session (the nonce is store state, not an event log). Acting on it pushed
        // a phantom entry and overwrote the line the entry had recorded — which is exactly the "回来
        // 一看行号没了" report. A jump is only news when the nonce CHANGES after that baseline.
        if (jump && seenJumpRef.current < 0) seenJumpRef.current = jump
        else if (jump && jump !== seenJumpRef.current) {
      ...
      // The active note can change without the reader touching the note picker: the agent switches it
      // from the host (`note_open`). An editor opened on the previous note must not survive that —
      // it used to stay on screen holding the OTHER note's text, and committing it would have spliced
      // that text into the note on screen. The renderer refuses to draw a foreign editor; this closes
      // it, which is the honest visible behaviour.
      React.useEffect(function () {
        const eb = editBlockRef.current
        const ce = cellEditRef.current
        if (eb && eb.note && eb.note !== noteName) { editBlockRef.current = null; setEditBlock(null) }
        if (ce && ce.note && ce.note !== noteName) { cellEditRef.current = null; setCellEdit(null) }
      }, [noteName])
      // The cell editor: focus it, put the caret where the reader clicked, and commit it when the
      // pointer goes anywhere else (the shell takes focus back on its own schedule, so a blur is not a
      // reliable "finished" signal).
      React.useEffect(function () {
        if (cellEdit === null) return undefined
        const input = cellInputRef.current
        try {
          if (input) {
            input.focus({ preventScroll: true })
            const at = Number.isFinite(cellEdit.caretAt) ? cellEdit.caretAt : input.value.length
            input.setSelectionRange(at, at)
          }
        } catch (err) { }
        const onDown = function (ev) {
          const node = ev.target
          const cur = cellInputRef.current
          if (cur && (node === cur || (node && cur.contains && cur.contains(node)))) return
          if (node && node.closest && node.closest('.dn-bar, .dn-handle')) return
          // A press on ANOTHER CELL. With 单击 (the click edits) the click handler hands the editor
          // over to that cell, so this one is left alone for a moment. With 双击 (the default) a single
          // click never opens an editor, so waiting for one meant the editor stayed open until the
          // reader clicked OUTSIDE the whole table — reported from a real table. Commit and close here
          // instead; a double click on the other cell still opens it (its own handler runs after).
          if (node && node.closest && node.closest('[data-cell-at]')) {
            if (clickToEditRef.current) return
            commitCellEdit(0)
            return
          }
          commitCellEdit(0)
        ...
        return function () { try { document.removeEventListener('pointerdown', onDown, true) } catch (err) { } }
      }, [cellEdit === null ? -1 : cellEdit.line * 1000000 + cellEdit.at])
       ...
       */
      /** The mark-list preview: a FRAGMENT, so `startLine` is what makes a box clickable and
       *  `live` says the row belongs to the note on screen (toggling writes to that note). */
      function markBlocks(text, startLine, live) {
        ...
        for (let i = 0; i < blocks.length; i++) {
          ...
          if (b.k === 'li') {
            const marker = b.task
              ? (live === true
                ? taskMarker(b, Math.max(1, Math.round(Number(startLine) || 1) + b.line - 1))
      ...
      /**
       * The ☐/☑ of a task item: a real button, because that is what a reader clicks. `line` is the
       * 1-based line of the note it belongs to, which is all the toggle needs (the source is rewritten
       * from `textRef`). Pointer events are stopped so a click here never starts a text selection.
       */
      function taskMarker(b, line) {
        return h('button', {
          className: 'dn-bullet dn-task', key: 'm', type: 'button', 'data-task': '1', 'data-line': line,
          title: b.checked ? '取消勾选（写回 - [ ]）' : '勾选（写回 - [x]）',
          onPointerDown: function (e) { e.stopPropagation() },
          onClick: function (e) { e.stopPropagation(); e.preventDefault(); toggleTaskAt(line) },
        }, b.checked ? '\u2611' : '\u2610')
      }
      /**
       * Toggle one task line of the note on disk. Same shape as 恢复原文: build the new text here, write
       * it through saveText (revision guard included), then update the card's copy of the note.
       */
      function toggleTaskAt(line) {
        const run = function () {
          const next = toggleTaskText(String(textRef.current || ''), line)
          if (next === null) { notify('这一行不是待办项（要写成 - [ ] 或 - [x]）'); return }
          setBusy('勾选中')
          host.call('saveText', { text: next, baseRevision: revRef.current, sessionId: sidRef.current }).then(function (r) {
            ...
            notify('勾选失败: ' + ((r && r.error) || '未知错误'))
          }).catch(function (err) { setBusy(''); notify('勾选失败: ' + ((err && err.message) || String(err))) })
        }
        // Unsaved edits would be overwritten by a write built from the old text, so they go first.
        if (dirtyRef.current) flush(true).then(function (ok) { if (ok) run() })
        else run()
      }
      /* STALE-PURE-START */
      /**
      ...
      function doReload() {
        ...
        host.call('reload', {}).then(function (r) {
          if (r && r.ok) {
            // The mirror may have changed under us (an outside edit can have added or replaced
            // images), so nothing cached from the old bytes is reused.
            clearAssetCache()
      ...
      function saveViewNow() {
        ...
        // The stack's current entry follows the reader (see stampNavLine): without this it kept the
        // line the note was OPENED with — 0 — until the session was left, so the stored stack showed
        // line: 0 for the note on screen and could not come back to the paragraph.
        stampNavLine(line)
        saveNavNow()
      ...
      /**
       * In-place TABLE CELL editing (Typora's gesture: click a cell, type, Enter).
       *
       * A table row is ONE source line, so editing a cell means "replace the characters of this cell
       * inside that line": the pipes, the padding, the alignment row and every other cell stay exactly
       * as they were. That is also why the commit re-reads the line and verifies that the cell still
       * holds the text it was opened with — if the note changed underneath (a poll with new content, an
       * outside edit), the offsets are stale and writing would land in the wrong place, so the commit
       * refuses and says so instead.
       */
      function startCellEdit(target, pt) {
        if (mode !== 'read') return false
        const cell = target && target.closest ? target.closest('[data-cell-at]') : null
        if (!cell) return false
        const numOf = function (raw, dflt) { const n = Number(raw); return Number.isFinite(n) ? n : dflt }
        const line = numOf(cell.getAttribute('data-cell-line'), 0)
        const at = numOf(cell.getAttribute('data-cell-at'), 0)
        const len = numOf(cell.getAttribute('data-cell-len'), 0)
        const ci = numOf(cell.getAttribute('data-cell-ci'), 0)
        if (line < 1) return false
        // Clicking another cell while one is being edited must COMMIT the first one first — otherwise
        // the buffer was simply dropped (measured: the typed text never reached the file).
        const open = cellEditRef.current
        if (open && (open.line !== line || open.at !== at)) commitCellEdit(0)
        const src = String(textRef.current || '').split(String.fromCharCode(10))[line - 1]
        if (typeof src !== 'string') return false
        const value = src.substr(at, len)
        // Where the caret goes: the character under the pointer, resolved from the rendered cell (its
        // spans carry the source offsets), so the caret lands where the reader clicked instead of at
        // the end of the cell.
        let caretAt = value.length
        if (pt && cell.querySelector) {
          try {
            const range = (document.caretRangeFromPoint && document.caretRangeFromPoint(pt.x, pt.y)) ||
              (document.caretPositionFromPoint && (function () { const p = document.caretPositionFromPoint(pt.x, pt.y); return p ? { startContainer: p.offsetNode, startOffset: p.offset } : null })())
            if (range && range.startContainer) {
              let node = range.startContainer
              let span = null
              while (node && node !== cell) { if (node.getAttribute && node.getAttribute('data-soff') !== null) { span = node; break } node = node.parentElement }
              if (span) caretAt = Math.max(0, Math.min(value.length, numOf(span.getAttribute('data-soff'), at) - at + numOf(range.startOffset, 0)))
            }
          } catch (err) { }
        }
        cellEditRef.current = { note: noteNameRef.current || '', line: line, at: at, len: len, ci: ci, value: value, wasText: value, caretAt: caretAt }
        setCellEdit(cellEditRef.current)
        return true
      }
      function bufferCellEdit(ev) {
        const ce = cellEditRef.current
        if (!ce) return
        cellEditRef.current = Object.assign({}, ce, { value: ev.target.value })
      }
      function cancelCellEdit() { cellEditRef.current = null; setCellEdit(null) }
      /**
       * Write the cell back. `dir` (+1/-1/0) also moves the caret to the neighbouring cell, which is what
       * Enter and Tab do in Typora; 0 just closes.
       */
      function commitCellEdit(dir) {
        ...
        setCellEdit(null)
        if (!ce || !st) return
        // The LIVE text, not the render-time snapshot: two commits can land in one tick (clicking the
        // next cell right after typing), and the second must not splice into the pre-first-commit text.
        const lines = String(textRef.current || st.text).split(String.fromCharCode(10))
        const src = lines[ce.line - 1]
        if (typeof src !== 'string') return
        if (src.substr(ce.at, ce.len) !== ce.wasText) {
          setToast('这个单元格已经被别处改过，没有写入（请重载后再改）')
          return
        }
        const value = String(ce.value).replace(/[\r\n]+/g, ' ')
        const next = src.slice(0, ce.at) + value + src.slice(ce.at + ce.len)
        const apply = function (text) {
          ...
          host.call('saveText', { text: text, baseRevision: revRef.current, sessionId: sidRef.current }).then(function (r) {
            ...
            if (r && r.ok) {
              ...
              setToast('已保存这个单元格')
              if (dir) nextCell(ce, value, dir)
            } else setToast(r && r.conflict ? '笔记已被别处修改，请先重载' : '保存失败')
        ...
        lines[ce.line - 1] = next
        const text = lines.join(String.fromCharCode(10))
        // Optimistic: the DOM updates from the new text right away, and the save follows.
        textRef.current = text
        setSt(function (prev) { return prev ? Object.assign({}, prev, { text: text }) : prev })
        apply(text)
      }
      /**
       * Move the cell editor to the next (+1) or previous (-1) cell. It walks to the neighbouring row
       * when the current row has no cell left, which is what Tab and Enter do in Typora (a table is a
       * grid, not a line).
       */
      function nextCell(ce, value, dir) {
        try {
          const rows = [...document.querySelectorAll('.dn-root .dn-tr[data-line]')]
          const at = rows.findIndex(function (tr) { return Number(tr.getAttribute('data-line')) === ce.line })
          if (at < 0) return
          for (let ri = at; ri >= 0 && ri < rows.length; ri += dir) {
            const cells = [...rows[ri].querySelectorAll('[data-cell-at]')]
            const here = cells.findIndex(function (c) { return Number(c.getAttribute('data-cell-at')) === ce.at })
            const from = here < 0 ? (dir > 0 ? 0 : cells.length - 1) : here + dir
            const to = cells[from]
            if (to) { startCellEdit(to, null); return }
          }
        } catch (err) { }
      }
      /**
      ...
      function startInlineEdit(pt) {
        ...
        const eb = {
          ...
          // …and which NOTE it belongs to. Nothing cleared this editor when the ACTIVE NOTE changed
          // from the host (the agent switching notes with note_open), so the box stayed on screen
          // holding the OTHER note's text — committing it would have spliced that text into the note
          // on screen. The editor now carries the note it was opened on, the renderer refuses to draw
          // it for another note, and an effect closes it when the note changes.
          note: noteNameRef.current || '',
      ...
      function onDoubleClick(e) {
        ...
        const lines = String(st.text).split(String.fromCharCode(10))
        const eb = { from: range.from, to: range.to, kind: range.kind, matchLine: range.from, note: noteNameRef.current || '', value: lines.slice(range.from - 1, range.to).join(String.fromCharCode(10)), caret: pt, openedAt: Date.now() }
        editBlockRef.current = eb
        setEditBlock(eb)
      }
      function headDown(e) {
      ...
      function renderTokens(toks, keyPrefix, line) {
        ...
        for (let i = 0; i < toks.length; i++) {
          const tk = toks[i]
          if (tk.k === 'img') {
            const img = h(NoteImage, { key: keyPrefix + 'img' + i, note: noteName, href: tk.href, alt: tk.t, soff: tk.base + tk.off, len: tk.len || (tk.t ? tk.t.length : 0), cls: 'dn-img' })
      ...
      function renderBlocks() {
        ...
        for (let bi = 0; bi < blocks.length; bi++) {
          const b = blocks[bi]
          const key = 'b' + bi
          // An editor opened on ANOTHER note is never drawn (see the `note` field): the card's note
          // can change from the host while an editor is open, and drawing it then showed the other
          // note's text in the box on screen.
          if (editBlock && (!editBlock.note || editBlock.note === noteName) && b.line === (editBlock.matchLine || editBlock.from)) {
            ...
            if (inPlace) {
              const caretOff = editBlock.caret
                ...
                : -1
              const shared = {
                // The note is part of the key: if it ever changes while this stays mounted, React
                // builds a fresh editor (with a fresh mirror) instead of reusing the old instance.
                key: key + '_' + (editBlock.note || '') + '_' + (editBlock.openedAt || 0),
                block: b, value: editBlock.value, caretOffset: caretOff, firstLine: editBlock.from,
                session: (editBlock.note || '') + ':' + editBlock.from + ':' + (editBlock.openedAt || 0),
          ...
          if (b.k === 'table') {
            ...
            for (let ri = 0; ri < b.rows.length; ri++) {
              ...
              for (let ci = 0; ci < cells.length; ci++) {
                ...
                const base = at < 0 ? search : at
                search = base + txt.length
                const editing = cellEdit && cellEdit.note === noteName && cellEdit.line === row.line && cellEdit.at === base
                kids.push(h(row.head ? 'th' : 'td', {
                  className: (row.head ? 'dn-th' : 'dn-td') + (editing ? ' dn-cell-edit' : ''),
                  key: 'd' + ci,
                  style: { textAlign: b.align && b.align[ci] ? b.align[ci] : 'left' },
                  // Where this cell's text starts in the SOURCE line and how long it is, so a click
                  // becomes "edit THIS cell" without re-parsing the row, and the commit can splice the
                  // characters back into exactly that span.
                  'data-cell-line': String(row.line),
                  'data-cell-at': String(base),
                  'data-cell-len': String(txt.length),
                  'data-cell-ci': String(ci),
                }, editing
                  ? [
                    // The cell's own rendering is only the SHAPE (it keeps the box from collapsing and
                    // keeps the column width); the characters being edited are drawn by the input,
                    // which is why it is visible and this is hidden.
                    h('span', { className: 'dn-cell-mirror', key: 'm' }, lineSpans(txt, base, false, row.line)),
                    h('input', {
                      className: 'dn-cell-in', key: 'i', ref: cellInputRef, defaultValue: cellEdit.value,
                      spellCheck: false, autoCapitalize: 'off', autoCorrect: 'off', 'aria-label': '编辑这个单元格',
                      onPointerDown: function (ev) { ev.stopPropagation() },
                      onDoubleClick: function (ev) { ev.stopPropagation() },
                      onInput: bufferCellEdit,
                      onKeyDown: function (ev) {
                        if (ev.key === 'Escape') { ev.preventDefault(); cancelCellEdit(); return }
                        if (ev.key === 'Enter') { ev.preventDefault(); commitCellEdit(ev.shiftKey ? -1 : 1); return }
                        if (ev.key === 'Tab') { ev.preventDefault(); commitCellEdit(ev.shiftKey ? -1 : 1); return }
                      },
                    }),
                  ]
      ...
      // The cell editor is part of the key: it is the only thing that changes when a cell is entered,
      // and without it the cached block list kept rendering the plain table (the click looked dead).
      const blocksKey = (st ? st.text : '') + '|' + mode + '|' + (editBlock ? editBlock.from + ':' + editBlock.to : '') + '|' + (cellEdit ? 'c' + cellEdit.line + ':' + cellEdit.at + ':' + cellEdit.len + ':' + cellEdit.note : '') + '|' + styleKeyOf(st)
      ...
      function markRow(m, noteName, cross, listName) {
        ...
        return h('div', {
          ...
          }, h('span', { className: (look.italic ? 'dn-mki' : '') + (look.underline ? ' dn-mku' : '') }, markBlocks(m.text, m.startLine, cross !== true))),
      ...
      if (menuOpen) {
        ...
        if (menuOpen.kind === 'notes') {
          ...
          // The recorded line is shown for EVERY entry that has one, not only for one inside the note
          // being read: it is the visible proof that the line really is in the stack (and in the stored
          // one), and it makes 后退/前进 read as "back to that paragraph" instead of "open that note".
          // An entry with no line yet (0) shows just the name.
          const navLabel = function (e) {
            if (!e) return ''
            return e.name + (e.line >= 1 ? '  · 第 ' + e.line + ' 行' : '')
          }
          ...
          items.push(mi('sync', '与来源文件夹重新同步', false, function () {
            ...
            host.call('syncNote', { sessionId: sidRef.current }).then(function (r) {
              ...
              // A sync copies files: an image that was missing a moment ago can exist now, so the
              // cached bytes (and the remembered failures) are dropped before the next render.
              clearAssetCache()
