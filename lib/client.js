...
ctx.effect(function () { return function () { try { if (ownStyle) ownStyle.remove() } catch (err) { } } })
    ...
    function NoteCard(props) {
      ...
      // Which imported folders are expanded in the note menu (keyed by asset root).
      ...
      React.useEffect(function () {
        ...
        if (nav.list[nav.at] && nav.list[nav.at].name === name) return
        nav.list = nav.list.slice(0, nav.at + 1)
        // An entry remembers the note AND the line the reader was on when they left it by jump:
        // back/forward must return to that spot, not to whatever the live scroll record says last.
        nav.list.push({ name: name, line: 0 })
      ...
      function onBodyLinkClick(e) {
        ...
        e.preventDefault()
        e.stopPropagation()
        // Record where the reader IS before the jump: back/forward returns here. This covers both a
        // link into another note and an anchor inside this one — the same handler serves both.
        (function () {
          const nav = navRef.current
          const cur = nav.at >= 0 ? nav.list[nav.at] : null
          if (cur && cur.name === String(noteNameRef.current || '')) { const at = topVisibleLine(); if (at) cur.line = at }
        })(),
      ...
      function navGo(delta) {
        ...
        // The recorded line is used ONLY here: a back/forward jump restores the exact spot the reader
        // left from, while ordinary opening still uses the live reading position as before.
        if (entry.line >= 1) {
      ...
      if (menuOpen) {
        ...
        if (menuOpen.kind === 'notes') {
            ...
            if (!items.__treeBuilt) {
              ...
              buildNoteMenuRows(notes, openGroups).forEach(function (r) {
                ...
                items.push(mi(r.key, (r.open ? '▾ ' : '▸ ') + r.label + (r.kind === 'folder' ? '  (' + r.count + ' 份)' : ''), false, function () {
                  setOpenGroups(function (prev) { const nx = Object.assign({}, prev); nx[r.key] = nx[r.key] !== true; return nx })
          ...
          items.push(mi('back', '← 后退' + (canBack ? '  →  ' + nav.list[nav.at - 1].name : ''), false, function () { navGo(-1) }, canBack ? '' : 'dn-menu-item-sub', undefined, 0, true))
          items.push(mi('fwd', '→ 前进' + (canFwd ? '  →  ' + nav.list[nav.at + 1].name : ''), false, function () { navGo(1) }, canFwd ? '' : 'dn-menu-item-sub', undefined, 0, true))
