...
'.dn-mark[data-color=black]{border-left-color:#20242c;}',
// A mark that lives in the text gets a rail of its own shape, so the two kinds are told apart
// before reading the badge.
'.dn-mark[data-style=italic]{border-left-style:dashed;border-left-color:#8b5cf6;}',
'.dn-mark[data-style=underline]{border-left-style:double;border-left-width:4px;border-left-color:#0ea5e9;}',
...
// The function card a single click raises on a mark. Fixed to the viewport, because it is
// anchored to the pointer rather than to any box, and it has to survive the card being dragged.
'.dn-mcard{position:fixed;z-index:80;display:flex;flex-direction:column;gap:6px;padding:7px 8px;background:var(--dsw-alias-bg-layer-1,#fff);color:var(--dsw-alias-label-primary,#1b1b1b);border:1px solid rgba(0,0,0,.14);border-radius:10px;box-shadow:0 10px 26px rgba(0,0,0,.22);font-size:12px;}',
...
function markStyle(m) { return m && (m.style === 'italic' || m.style === 'underline') ? m.style : 'highlight' }
/**
...
function textStyleRuns(sels) {
  ...
  for (let i = 0; i < sels.length; i++) {
    const s = sels[i]
    const style = markStyle(s)
    if (style === 'highlight') continue
    key.push(s.id + ':' + style + ':' + s.startLine + ',' + s.startCol + '-' + s.endLine + ',' + s.endCol)
    for (let ln = s.startLine; ln <= s.endLine; ln++) {
      ...
      byLine[ln].push({ from: from, to: to, style: style, id: s.id })
    }
  }
...
ctx.effect(function () { return function () { try { if (ownStyle) ownStyle.remove() } catch (err) { } } })
    ...
    function NoteCard(props) {
      ...
      // What the next mark will be: a translucent wash, or the text itself changed. Kept next to
      // the colour because the bar offers both in one place, and remembered for the session.
      const [penStyle, setPenStyle] = React.useState('highlight')
      ...
      function commitLive(remark) {
        ...
        host.call('addSelection', Object.assign({ color: penColor, style: markStyle({ style: penStyle }), sessionId: sidRef.current, remark: typeof remark === 'string' ? remark : '' }, rangeArgs)).then(function (r) {
          if (r && r.ok) {
            revRef.current = r.revision
            ...
            const how = penStyle === 'italic' ? '（斜体）' : (penStyle === 'underline' ? '（下划线）' : '')
      ...
      /**
       * Long press (or drag) on the list's header lifts it out of the note card. A plain press
       * that neither moves far nor lasts long is left alone, so the header stays clickable.
       */
      ...
      function moveMarksLift(e) {
        ...
        d.x = e.clientX
        d.y = e.clientY
        const far = Math.abs(e.clientX - d.x0) + Math.abs(e.clientY - d.y0) > 14
        if (far) d.moved = true
        if (d.moved || Date.now() - d.t > 380) {
          marksLiftRef.current = null
          liftMarksOut({ x: e.clientX - 40, y: Math.max(8, e.clientY - 14) })
        }
      }
      ...
      /** Change a mark's colour or its style from the card. */
      function applyLook(m, color, style) {
        if (!m) return
        const args = { sessionId: sidRef.current, id: m.id }
      ...
      function onBodyClick(e) {
        ...
        mcardTimerRef.current = window.setTimeout(function () {
          mcardTimerRef.current = null
          setMcard({ id: m.id, x: Math.round(x + 6), y: Math.round(y + 10) })
          revealMarkInList(m)
        }, 210)
      }
      ...
      function styleRunsFor(line) {
        ...
        const text = String(textRef.current || '')
        const key = (cur ? cur.revision : -1) + '|' + text.length + '|' + sels.length
      ...
      if (mode === 'read') {
        ...
        for (let i = 0; i < selList.length; i++) {
          const s = selList[i]
          // An italic / underline mark draws itself in the glyphs (see renderTokens), so it gets
          // no wash behind it: two marks of different styles on the same words must stay
          // readable as two marks, and a tint over italic text would hide which is which.
          if (markStyle(s) !== 'highlight') continue
          for (let ln = s.startLine; ln <= s.endLine; ln++) {
            if (!byLine[ln]) byLine[ln] = []
      ...
      if (live && mode === 'read') {
        ...
        if (anchor && barReady) {
          ...
          layFront.push(h('div', { className: 'dn-bar', key: 'bar', style: { left: Math.max(4, anchor.x - 10) + 'px', top: barTop + 'px', pointerEvents: ovPe } }, [
            ...
            // The two marks that live in the text instead of behind it. Drawn to the right of
            // [标记] because they commit the SAME way — only the chosen look differs.
            h('span', { className: 'dn-pen', key: 'style' }, [
              h('button', {
                className: 'dn-pen-sw', key: 'it', 'data-c': 'italic', 'data-on': penStyle === 'italic' ? 'true' : 'false',
                title: '下一处标记用斜体（直接改文字，不铺底色）',
                onPointerDown: press(function () { setPenStyle(penStyle === 'italic' ? 'highlight' : 'italic') }),
                onClick: tap(function () { setPenStyle(penStyle === 'italic' ? 'highlight' : 'italic') }),
              }, 'I'),
              h('button', {
                className: 'dn-pen-sw', key: 'ul', 'data-c': 'underline', 'data-on': penStyle === 'underline' ? 'true' : 'false',
                title: '下一处标记用下划线',
                onPointerDown: press(function () { setPenStyle(penStyle === 'underline' ? 'highlight' : 'underline') }),
                onClick: tap(function () { setPenStyle(penStyle === 'underline' ? 'highlight' : 'underline') }),
              }, 'U'),
            ]),
          ]))
        }
      ...
      const actions = h('div', { className: 'dn-actions', key: 'actions' }, [
        ...
        btn('标记' + (selList.length ? ' ' + selList.length : ''), function () { const willOpen = !panel; setPanel(willOpen); if (willOpen) loadAllMarks() }, { active: panel, title: '查看/管理标记（本笔记 / 本会话）' }),
        btn('清空', function () { clearMarks() }, { title: '清空全部标记' }),
      ...
      function markRow(m, noteName, cross) {
        const key = (cross ? noteName + '/' : '') + m.id
        ...
        return h('div', {
          ...
          key: key, 'data-color': m.color || 'yellow', 'data-style': style,
          ...
          h('div', { className: 'dn-mark-top', key: 't' }, [
            ...
            style === 'italic' ? h('span', { className: 'dn-mark-badge', key: 'si' }, '斜体') : null,
            style === 'underline' ? h('span', { className: 'dn-mark-badge', key: 'su' }, '下划线') : null,
            ...
            h('span', { className: 'dn-mark-acts', key: 'a' }, [
              h('button', { className: 'dn-mact', key: 'go', title: '跳到正文这一行', onClick: function (e) { e.stopPropagation(); jumpToMark(cross ? noteName : '', m.startLine) } }, '跳到'),
              h('button', {
              ...
              cross ? null : h('button', {
                className: 'dn-mact', key: 'look', title: '改颜色 / 改斜体下划线 / 复制 / 备注',
                onClick: function (e) {
                  e.stopPropagation()
                  const r = e.currentTarget.getBoundingClientRect()
                  setMcard({ id: m.id, x: Math.round(r.left), y: Math.round(r.bottom + 6), from: 'list' })
                },
              }, '样式'),
              cross ? null : h('button', {
              ...
              cross ? null : h('button', { className: 'dn-mact dn-mact-del', key: 'x', title: '删除这条标记', onClick: function (e) { e.stopPropagation(); removeSelection(m.id) } }, '删除'),
            ]),
          ]),
          h('div', {
            className: 'dn-mark-body', key: 'b', title: '点这里跳到正文',
            onClick: function (e) { e.preventDefault(); e.stopPropagation(); jumpToMark(cross ? noteName : '', m.startLine) },
          }, h('span', { className: style === 'italic' ? 'dn-mki' : (style === 'underline' ? 'dn-mku' : '') }, markBlocks(m.text))),
          m.remark ? h('div', { className: 'dn-mark-remark', key: 'r' }, ['备注：', mdInline(m.remark)]) : null,
        ])
      }
      const markRows = markTab === 'session'
      ...
      const marksHeadEl = h('div', {
        ...
        // Long press here (or drag it) lifts the list out of the card into its own window.
        onPointerDown: function (e) {
          if (marksWin) startMarksWinDrag(e, 'move')
          else startMarksLift(e)
        },
        onPointerMove: function (e) { if (marksWin) moveMarksWinDrag(e); else moveMarksLift(e) },
        onPointerUp: function (e) { if (marksWin) endMarksWinDrag(); else endMarksLift(e) },
        onPointerCancel: function () { if (marksWin) endMarksWinDrag(); else endMarksLift(null) },
      }, [
        h('span', { className: 'dn-marks-title', key: 't' }, marksWin ? '标记（独立窗口）' : '标记'),
        h('span', { className: 'dn-tabs', key: 'tabs' }, [
          h('button', { className: 'dn-tab' + (markTab === 'note' ? ' dn-tab-on' : ''), key: 'n', title: '当前打开的这份笔记', onClick: function () { setMarkTab('note') } }, '本笔记 ' + selList.length),
          h('button', {
            className: 'dn-tab' + (markTab === 'session' ? ' dn-tab-on' : ''), key: 's', title: '本会话所有笔记的所有标记',
            onClick: function () { setMarkTab('session'); loadAllMarks() },
          }, '本会话 ' + sessionMarks),
        ]),
        h('button', {
        ...
        h('button', {
          ...
          title: marksWin ? '收回卡片内' : '拖出为独立小窗（长按标题栏也能拖出）',
          onClick: function () { if (marksWin) dockMarksBack(); else liftMarksOut(null) },
        }, marksWin ? '收回' : '拖出'),
        h('button', { className: 'dn-marks-refresh', key: 'r', title: '刷新列表', onClick: function () { loadAllMarks() } }, '刷新'),
      ])
      ...
      const mcardEl = mcard ? h('div', {
        className: 'dn-mcard', key: 'mcard', ref: mcardRef,
        style: {
          left: Math.round(clamp(mcard.x, 6, Math.max(6, (bounds.w || 1200) - 240))) + 'px',
          top: Math.round(clamp(mcard.y, 6, Math.max(6, (bounds.h || 800) - 120))) + 'px',
        },
        onPointerDown: function (e) { e.stopPropagation() },
      }, [
        h('div', { className: 'dn-mcard-row', key: 'sw' }, [
          ...
          ...PEN_COLORS.map(function (c) {
            const on = cardMark && markStyle(cardMark) === 'highlight' && (cardMark.color || 'yellow') === c
            return h('button', {
              className: 'dn-mcard-sw', key: c, 'data-c': c, 'data-on': on ? '1' : '0', title: '底色：' + c,
              onClick: function () { applyLook(cardMark, c, 'highlight') },
            })
          }),
          h('span', { className: 'dn-mcard-sep', key: 'sep' }),
          h('button', {
            className: 'dn-mcard-sw', key: 'it', title: '斜体标记（改文字本身，不铺底色）',
            'data-on': cardMark && markStyle(cardMark) === 'italic' ? '1' : '0',
            style: { fontStyle: 'italic', fontFamily: 'Georgia,serif' },
            onClick: function () { applyLook(cardMark, null, 'italic') },
          }, 'I'),
          h('button', {
            className: 'dn-mcard-sw', key: 'ul', title: '下划线标记',
            'data-on': cardMark && markStyle(cardMark) === 'underline' ? '1' : '0',
            style: { textDecoration: 'underline', textDecorationThickness: '1.5px' },
            onClick: function () { applyLook(cardMark, null, 'underline') },
          }, 'U'),
        ]),
        h('div', { className: 'dn-mcard-row', key: 'acts' }, [
          ...
          h('button', { className: 'dn-mcard-btn', key: 'go', title: '跳到正文这一行', onClick: function () { jumpToMark('', cardMark ? cardMark.startLine : 1); setMcard(null) } }, '跳到'),
      ...
      return h('div', { className: 'dn-root', ref: rootRef, style: geo.style || undefined, 'data-panel-mode': geo.mode }, [
        ...
        panelEl, foot, modalEl, remarkEl, mcardEl,
