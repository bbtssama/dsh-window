...
ctx.effect(function () { return function () { try { if (ownStyle) ownStyle.remove() } catch (err) { } } })
    ...
    function NoteCard(props) {
       ...
       */
      function navSwapSession(store, fromSid, toSid, current) {
        const from = String(fromSid || '')
        const to = String(toSid || '')
        if (from === to) return { store: store, nav: current }
        const next = Object.assign({}, store)
        if (from !== '') next[from] = current
        return { store: next, nav: next[to] || { list: [], at: -1 } }
      }
      /* NAV-PURE-END */
      ...
      if (sidRef.current !== lastSidRef.current) {
        ...
        // The visit history belongs to the session being left, not to this one: park it and pick
        // up this session's own (which is empty the first time). Everything that pointed into the
        // other session is dropped with it — a target note, a pending recorded line, the jump
        // nonce that came from that store.
        const swapped = navSwapSession(navBySidRef.current, navSidRef.current, sidRef.current, navRef.current)
