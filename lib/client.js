...
// covered characters cannot be read. Declared after the stale/live rules so it
  ...
  ctx.effect(function () { return function () { try { styleTag.remove() } catch (err) { } } })
    ...
    function NoteCard(props) {
      ...
      React.useEffect(function () {
        const host = bodyRef.current
        if (!host) return undefined
        const bar = host.querySelector('.dn-bar')
      ...
      function onDragMove(ev) {
        ...
        if (d.unit) {
          ...
          // A long press picks a unit (a word, an image's source, a whole block) and it
          // has to STAY that unit. Moving vertically is the body's own scroll gesture
          // (`.dn-body{touch-action:pan-y}`), and on a phone a press-and-drag is how you
          // scroll — treating that as "extend the range" is what produced the reported
          // "long press on the blue area selects a huge span": the selection followed the
          // finger down the document. Only a deliberate horizontal drag extends now, and
          // a block never does (dragging out of a diagram means nothing). Extending a
          // range on purpose is still done with the two handles, which is exact.
          if (dy > UNIT_SLOP_Y || dx <= UNIT_SLOP_X || d.unit === 'block') return
          d.unit = null
        }
