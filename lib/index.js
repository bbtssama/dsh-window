...
function normalizeSel(raw) {
  ...
  return {
    ...
    // WHO made this mark (review §P0-4). The agent's own annotations used to come back to it as
    // "the user just marked this", which is both wrong and a permanently-consumed user signal; old
    // records have no field and default to "user" — better to over-report a user mark than to lose one.
    author: raw.author === 'agent' ? 'agent' : 'user',
  }
}
...
export function apply(ctx, config) {
  ...
  void config
    ...
    //   ui    a queued view command (note_card_ui / note_card_panel)
    let events = []
    let eventSeq = 0
    ...
    async function selectNote(name) {
      ...
      S.gitTrace = ''
      S.touched = false
      // The increment baseline belongs to ONE note, and this reset used to leave it behind: switching
      // to a note whose state file does not exist yet kept the PREVIOUS note's `lastRead`, so
      // note_status answered "自你上次读取（rev 4，0 秒前）" for a note nobody had ever read.
      // (Found by the live acceptance run; the unit suite always had a state file to parse.)
      S.lastRead = null
      S.unreadText = ''
      S.error = ''
      stateCorrupt = false
    ...
    async function load() {
      ...
      S.view = null
      S.fileExists = false
      // The increment baseline belongs to ONE note and must never survive a load: a note whose state
      // file does not exist yet used to inherit the PREVIOUS note's `lastRead`, so note_status answered
      // "自你上次读取（rev 4，0 秒前）" for a note nobody had ever read, and a stale snapshot could even
      // make it claim "无改动". (Found by the live acceptance run; the unit suite always had a state
      // file to parse, so this never showed up there.)
      S.lastRead = null
      S.unreadText = ''
      ...
      try {
        ...
        if (rawState !== null) {
          ...
          if (ok && parsed && Array.isArray(parsed.selections)) {
            ...
            // The agent-read baseline (review §P0-2). Validated the same way as a mark: a damaged
            // record simply means "no baseline", which is a normal state (first read).
            S.lastRead = parsed.lastRead && typeof parsed.lastRead === 'object' ? parsed.lastRead : null
            // The §P1-5 migration is a ONE-TIME question ("does this repo still track the state
            // file?"). Remembering the answer in the state file keeps it out of every later
            // activation: the probe costs a git process, and a note switch happens often.
            S.stateUntracked = parsed.stateUntracked === true
          } else if (!ok) {
            stateCorrupt = true
      ...
      // One cheap diff per load, so the per-turn prompt variable has a summary to print (it runs
      // outside the store activation and cannot compute one itself).
      await refreshUnread()
    }
    ...
    async function persistState() {
      ...
      emit('marks', { revision: S.revision })
      // v2 adds the increment protocol (§7.1): the content identity, the head commit and the agent
      // baseline. All three are additive — a reader that does not know them simply ignores them.
      const payload = { v: STATE_VERSION, seq: S.seq, selections: S.selections, updatedAt: isoNow(), contentSha: contentHashOf(S.text), headRev: S.commitHash || '', lastRead: S.lastRead || null, stateUntracked: S.stateUntracked === true }
     ...
     */
    /**
     * Keep the reading position AND the mark state out of the note's git history (review §P1-5).
     *
     * The note repo commits with `git add -A`, so every scroll and every mark edit became a commit:
     * the state file (23 KB for 32 marks) is rewritten on each mark change, which drowned the real
     * content history and permanently defeated the "skip an empty commit" check. Both files are
     * ignored now, and a repository that still TRACKS the state file is migrated once per process.
     */
    async function ensureViewIgnored() {
      ...
      try {
        ...
        const want = []
        if (cur.indexOf(VIEW_FILE) < 0) want.push(VIEW_FILE)
        if (cur.indexOf(STATE_FILE) < 0) want.push(STATE_FILE)
        if (want.length) {
          const head = cur === '' ? '' : cur.replace(/\s*$/, '') + '\n'
          await writeAt(gi, head + want.join('\n') + '\n')
        }
        if (S.stateChecked === true) return
        S.stateChecked = true
        if (S.stateUntracked === true) return
        if (!activeNote || fs === undefined) return
        if (!S.gitReady) S.gitReady = await noteHasRepo(paths().dir)
        if (!S.gitReady) return
        const tracked = await runGit('ls-files --error-unmatch ' + STATE_FILE)
        if (tracked.ok === true) {
          await runGit('rm --cached -q ' + STATE_FILE)
          // One commit records the migration; the state file itself stays on disk, just untracked.
          try { await commit('note(state): 停止跟踪 ' + STATE_FILE + '（批注状态不再污染内容历史）') } catch (err) { }
        }
    ...
    async function saveView(input) {
      ...
      // An announcement, not a plain scroll save: only a JUMP asks the card to move (note_scroll_to),
    ...
    function viewSelections() {
      ...
      for (let i = 0; i < S.selections.length; i++) {
        ...
        out.push({ id: s.id, order: i + 1, seq: s.seq, text: s.text, startLine: s.startLine, startCol: s.startCol, endLine: s.endLine, endCol: s.endCol, createdAt: s.createdAt, color: s.color || 'yellow', italic: look.italic, underline: look.underline, style: styleLabel(look.italic, look.underline), fetched: s.fetched === true, stale: s.stale === true, author: s.author === 'agent' ? 'agent' : 'user', remark: s.remark || '' })
      }
      return out
    }
    ...
    async function addSelection(input) {
      ...
      S.seq += 1
      const sel = { id: 'sel-' + S.seq, seq: S.seq, startLine: first.line, startCol: first.col, endLine: last.line, endCol: last.col, text: text, createdAt: isoNow(), color: COLORS[a.color] ? a.color : 'yellow', italic: lookFlags(a).italic, underline: lookFlags(a).underline, fetched: false, stale: false, remark: cleanRemark(a.remark), author: a.author === 'agent' ? 'agent' : 'user' }
    ...
    async function setMarkLook(id, patch) {
      ...
      // The retired single field still arrives from an older client (or the note_mark_style tool).
    ...
    /**
     * 「确认变动」 on a stale mark: the reader is looking at the orange band and says "this is what I
     * meant". The mark keeps the range it now has, the CURRENT text under it becomes what it
     * remembers, and `stale` clears — which is what makes the warning go away. The note is untouched.
     */
    async function confirmStaleSelection(id) {
      await ensureLoaded()
      const want = String(id || '')
      ...
      for (let i = 0; i < S.selections.length; i++) {
        ...
        if (s.id !== want) continue
        found = true
        const l1 = Math.min(Math.max(1, intOr(s.startLine, 1)), lines.length)
        const c1 = Math.max(0, intOr(s.startCol, 0))
        const l2 = Math.min(Math.max(l1, intOr(s.endLine, l1)), lines.length)
        const c2 = Math.max(0, intOr(s.endCol, c1))
        const now = sliceRange(S.text, { startLine: l1, startCol: c1, endLine: l2, endCol: c2 })
        if (now.trim() === '') return { ok: false, found: true, error: '这条标记现在盖住的是空白，没法确认（可以删掉重标）', revision: S.revision, selections: viewSelections() }
        s.startLine = l1; s.startCol = c1; s.endLine = l2; s.endCol = c2
        s.text = now
        s.stale = false
        outText = now
        S.revision += 1
      ...
      if (!found) return { ok: false, found: false, error: '没有这条标记记录: ' + want, revision: S.revision, selections: viewSelections() }
      try { await persistState() } catch (err) { fail('写入标记记录', err) }
      return { ok: true, found: true, id: want, text: outText, revision: S.revision, selections: viewSelections() }
    }
    ...
    // from the list's own 添加到 button, or by the agent through note_mark_lists — same store, same
    // rules, one code path.
    ...
    async function commit(message) {
      ...
      // Before `add -A`: the mark state file must be ignored (and untracked once), or a batch of
      // mark edits keeps creating content commits that contain no content (review §P1-5).
      await ensureViewIgnored()
      try { await writeAt(paths().note, S.text); await persistState() } catch (err) { fail('提交前落盘', err) }
      const status = await runGit('status --porcelain')
      if (status.ok && status.out === '') {
        // Nothing to record, but the marks ARE recorded (the note repo is clean, and the state file is
        // ignored by design §P1-5), so the "uncommitted" hint has to clear here too — this early return
        // is what the live acceptance run hit: the commit answered "没有需要提交的改动" and note_diag
        // still claimed uncommitted marks.
        S.touched = false
        return { ok: true, nothing: true, hash: S.commitHash, message: '没有需要提交的改动' }
      }
      const add = await runGit('add -A')
      if (!add.ok) return { ok: false, error: 'git add 失败: ' + (add.err || add.out) }
      // P1-6: a commit message that says what changed. The old auto form was a timestamp, which threw
      // away the cheapest increment signal there is — one `git log` and the agent knows the scale of
      // the change without calling a tool. An explicit message (the user's, or note_write's
      // `note(ai): …`) is left exactly as it was: its author already said what it is.
      let autoMsg = ''
      if (!message || !String(message).trim() || /^note:\s/.test(String(message))) {
        let linesText = ''
        try {
          const short = await runGit('diff --shortstat HEAD')
          if (short.ok && short.out) linesText = String(short.out).replace(/\s+/g, ' ').trim()
        } catch (err) { linesText = '' }
        const marks = S.selections.length
        autoMsg = 'note(content): ' + (linesText || 'first commit') + ', 标记 ' + marks + ' 条 (rev ' + S.revision + ')'
      }
      const msg = String(autoMsg || message).replace(/"/g, "'")
      ...
      // A commit records the marks as they are right now, so "标记有未提交改动" is false again. It used
      // to stay true for the rest of the activation (only a note switch cleared it), which made that
      // line of note_diag meaningless right after a successful commit. (Found by the live acceptance
      // run: note_commit answered a hash and note_diag immediately claimed uncommitted marks.)
      S.touched = false
     ...
     */
    // ── 增量协议：rev / delta / base（评审 §7.1）───────────────────────────────────
    /**
     * A stable content identity for the note text: FNV-1a over whitespace-normalised text plus the
     * length. No crypto service and no subprocess, and "only reflowed whitespace" does not count as
     * a change — which is what keeps `note_status` honest about "nothing happened".
     */
    ...
    /** Where the "what the agent last read" snapshot lives: outside every note repository. */
    function baseDirPath() { return sessionRoot() + '/.bases' }
    function basePathOf(note) { return baseDirPath() + '/' + (sanitizeNoteName(note) || 'note') + '.md' }
    function textLines(t) { return String(t === undefined || t === null ? '' : t).split('\n') }
    /**
     * A line diff between the baseline snapshot and the current text.
     *
     * Common prefix/suffix are trimmed first (the common case: a paragraph changed in the middle),
     * then the remaining block goes through an LCS table when it is small enough to be worth it. A
     * block too large for that is reported as ONE replacement rather than being silently mis-diffed:
     * an honest summary beats a wrong hunk list.
     */
    function lineDiffOf(oldText, newText, opts) {
      const maxCells = intOr((opts || {}).maxCells, 160000)
      ...
      const rows = []
      const big = midA.length * midB.length > maxCells
      if (!big) {
        ...
        let i = 0, j = 0
        while (i < n && j < m) {
          if (midA[i] === midB[j]) { rows.push({ t: ' ', old: i, text: midA[i] }); i++; j++ }
          else if (dp[i + 1][j] >= dp[i][j + 1]) { rows.push({ t: '-', old: i, text: midA[i] }); i++ }
          else { rows.push({ t: '+', old: -1, text: midB[j] }); j++ }
        }
        while (i < n) { rows.push({ t: '-', old: i, text: midA[i] }); i++ }
        while (j < m) { rows.push({ t: '+', old: -1, text: midB[j] }); j++ }
      }
      let added = 0, removed = 0, charsAdded = 0, charsRemoved = 0
      if (big) {
        for (let i = 0; i < midB.length; i++) { added++; charsAdded += midB[i].length + 1 }
        for (let i = 0; i < midA.length; i++) { removed++; charsRemoved += midA[i].length + 1 }
      } else {
        for (let i = 0; i < rows.length; i++) { const r = rows[i]; if (r.t === '+') { added++; charsAdded += r.text.length + 1 } else if (r.t === '-') { removed++; charsRemoved += r.text.length + 1 } }
      }
      return {
        prefix: p, suffix: s, rows: rows, tooBig: big,
        linesAdded: added, linesRemoved: removed, charsAdded: charsAdded, charsRemoved: charsRemoved,
        changed: added > 0 || removed > 0, oldLines: a.length, newLines: b.length,
      }
    }
    /** Unified-style hunks with `context` unchanged lines around each change. */
    function hunksOf(diff, context, maxHunks) {
      const ctx = Math.max(0, intOr(context, 2))
      const cap = Math.max(1, intOr(maxHunks, 6))
      const marks = []
      for (let i = 0; i < diff.rows.length; i++) if (diff.rows[i].t !== ' ') marks.push(i)
      if (!marks.length) return []
      const spans = []
      let from = Math.max(0, marks[0] - ctx), to = Math.min(diff.rows.length - 1, marks[0] + ctx)
      for (let k = 1; k < marks.length; k++) {
        const want = Math.max(0, marks[k] - ctx)
        if (want <= to + 1) { to = Math.min(diff.rows.length - 1, marks[k] + ctx); continue }
        spans.push([from, to]); from = want; to = Math.min(diff.rows.length - 1, marks[k] + ctx)
      }
      spans.push([from, to])
      const out = []
      for (let i = 0; i < spans.length && out.length < cap; i++) {
        let newLine = 0
        for (let k = 0; k < spans[i][0]; k++) if (diff.rows[k].t !== '-') newLine++
        out.push({ atLine: diff.prefix + newLine + 1, rows: diff.rows.slice(spans[i][0], spans[i][1] + 1) })
      }
      return out
    }
    /** The baseline (what the agent last read) as stored in this note's state file. */
    function lastReadOf() {
      const r = S.lastRead
      if (!r || typeof r !== 'object') return null
      return {
        rev: intOr(r.rev, 0), sha: typeof r.sha === 'string' ? r.sha : '',
        at: typeof r.at === 'string' ? r.at : '', lines: intOr(r.lines, 0), chars: intOr(r.chars, 0),
        markIds: typeof r.markIds === 'string' ? r.markIds : '',
      }
    }
    /** Read the snapshot the baseline was taken from. null = "no baseline for this note yet". */
    async function readBaseSnapshot() {
      if (!activeNote || fs === undefined) return null
      try { return await readIfExists(basePathOf(activeNote)) } catch (err) { return null }
    }
    /** Remember "the agent has now seen THIS text" — the only writer of the baseline. */
    async function setLastRead(text) {
      const t = String(text === undefined || text === null ? S.text : text)
      S.lastRead = { rev: S.revision, sha: contentHashOf(t), at: isoNow(), lines: textLines(t).length, chars: t.length, markIds: S.selections.map(function (s) { return s.id }).join(',') }
      if (fs !== undefined && canWrite() && activeNote) {
        try { await writeAt(basePathOf(activeNote), t) } catch (err) { fail('写入增量基线', err) }
      }
      await refreshUnread()
      try { await persistState() } catch (err) { }
    }
    /** Marks added/removed since the baseline (ids, cheap and exact). */
    function markDelta() {
      const base = lastReadOf()
      const now = S.selections.map(function (s) { return s.id })
      if (!base) return { added: now.length, removed: 0, known: false }
      const was = base.markIds === '' ? [] : base.markIds.split(',')
      const had = {}, has = {}
      for (let i = 0; i < was.length; i++) had[was[i]] = true
      for (let i = 0; i < now.length; i++) has[now[i]] = true
      let added = 0, removed = 0
      for (let i = 0; i < now.length; i++) if (had[now[i]] !== true) added++
      for (let i = 0; i < was.length; i++) if (has[was[i]] !== true) removed++
      return { added: added, removed: removed, known: true }
    }
    /** The one-line header answers can carry (§7.1): ≈60 B, and it removes a whole class of "let me read the file". */
    function statusHead() {
      return '[note=' + (activeNote || '无') + ' rev=' + S.revision + ' git=' + (S.commitHash || 'none') + ' 行=' + textLines(S.text).length + ' 标记=' + S.selections.length + ']'
    }
    /**
     * What changed since the agent last read this note: the numbers, and (when asked) the hunks. The
     * baseline is per note and per session, exactly like every other piece of state here.
     */
    async function unreadOf(opts) {
      const o = opts || {}
      const base = lastReadOf()
      if (!base) return { known: false, first: true }
      const snapshot = await readBaseSnapshot()
      if (snapshot === null) return { known: false, first: true }
      const diff = lineDiffOf(snapshot, S.text)
      const marks = markDelta()
      const out = {
        known: true, first: false, rev: base.rev, sha: base.sha, at: base.at,
        linesAdded: diff.linesAdded, linesRemoved: diff.linesRemoved,
        charsAdded: diff.charsAdded, charsRemoved: diff.charsRemoved,
        marksAdded: marks.added, marksRemoved: marks.removed,
        changed: diff.changed || marks.added > 0 || marks.removed > 0,
        diff: diff,
      }
      if (o.hunks) out.hunks = hunksOf(diff, o.context, o.maxHunks)
      return out
    }
    /** Keep a one-line summary on the store record, so the sync prompt variable can print it. */
    async function refreshUnread() {
      try {
        const u = await unreadOf()
        S.unreadText = u.known !== true ? '' : (u.changed ? deltaText(u) : '无改动')
      } catch (err) { S.unreadText = '' }
    }
    /** "3 分钟前" — the age of the baseline, for the status line. */
    ...
    /** The delta as text: "+12/-0 行, +820/-0 字符, 标记 +2". */
    function deltaText(u) {
      if (!u || u.known !== true) return '（还没有基线：这是首次读取）'
      const parts = ['+' + u.linesAdded + '/-' + u.linesRemoved + ' 行', '+' + u.charsAdded + '/-' + u.charsRemoved + ' 字符']
      if (u.marksAdded || u.marksRemoved) parts.push('标记 +' + u.marksAdded + '/-' + u.marksRemoved)
      return parts.join(', ')
    }
    /** Render hunks compactly: `  12| text` for context, `- 12| …`, `+    | …`. */
    function hunksRender(hunks, capRows) {
      const cap = Math.max(1, intOr(capRows, 40))
      const out = []
      let shown = 0
      for (let i = 0; i < hunks.length && shown < cap; i++) {
        out.push('@@ 第 ' + hunks[i].atLine + ' 行起')
        const rows = hunks[i].rows
        for (let k = 0; k < rows.length && shown < cap; k++) {
          const r = rows[k]
          const text = r.text.length > 100 ? r.text.slice(0, 100) + '…' : r.text
          out.push((r.t === ' ' ? '  ' : r.t + ' ') + text)
          shown++
        }
      }
      return out.join('\n')
    ...
    /** Text render of the custom lists, for note_mark_lists and the tool card. */
    function listRender(listsView) {
      if (!listsView || listsView.length === 0) return '（本会话还没有自定义列表。用 note_mark_lists action=create name=… 建一个）'
      const parts = []
    ...
    const SEL_ITEM = {
      ...
      properties: {
        ...
        // Who made this mark: 'user' (the reader highlighted it) or 'agent' (this plugin's own
        // note_mark_add). Without it, a mark the agent created while answering came back on the next
        // turn as if the reader had just highlighted it (review §P0-4).
    ...
    async function createNote(input) {
      ...
      if (a.open !== false) {
        ...
        // the caller is bulk-creating notes (the card's 新建 button, a 12-note loop, an import).
        // The note_create TOOL builds it eagerly instead — see its execute (review §P1-7).
        await load()
    ...
    /** Remove a directory tree (note_delete_forever). Prefers the fs service, falls back to shell. */
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        await enterFromTool('note_mark_list', exec)
    ...
    registerToolLocked(defineTool({
      name: 'note_mark_new',
      description: '领取用户自上次领取之后新划选的内容（默认只取 user 划的，并立即标记为已取用）。回答用户问题前先调用它。返回默认 brief；要某几条的备注与原文用 note_mark_list({ids, detail:"full"})。想再看一遍已取用过的（例如上下文被压缩后重新对齐）传 redeliver:true。',
      parameters: {
        note: { type: 'string', description: '取哪一份笔记的新标记（默认当前打开的）' },
        author: { type: 'string', enum: ['user', 'agent', 'all'], description: '取谁的标记：user（默认，读者划的）/ agent（你自己之前加的）/ all。' },
        redeliver: { type: 'boolean', description: 'true = 连之前已取用过的也再给一次，且不消耗取用状态（可反复领取）。' },
      },
      output: {
        schema: { type: 'array', items: SEL_ITEM },
        render: function (a, v) {
          const who = a && a.author === 'agent' ? 'agent' : (a && a.author === 'all' ? 'all' : 'user')
          const head = who === 'user' ? '用户新选中了 ' : (who === 'agent' ? '你之前加的标记 ' : '标记 ')
          const tail = a && a.redeliver === true ? '（redeliver：含已取用过的）' : ''
          return [{ type: 'text', text: v.length ? (head + v.length + ' 段' + tail + ':\n' + selRender(v)) : ('(没有' + (who === 'user' ? '新的选中内容' : '符合条件的标记') + ')') }]
        },
      },
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_mark_new', exec)
        return await useNote(args && args.note, async function () {
        markTouched()
        const who = args && (args.author === 'agent' || args.author === 'all') ? args.author : 'user'
        const redo = !!(args && args.redeliver === true)
        const wants = {}
        const consume = {}
        let count = 0
        for (let i = 0; i < S.selections.length; i++) {
          const s = S.selections[i]
          const a = s.author === 'agent' ? 'agent' : 'user'
          if (who !== 'all' && a !== who) continue
          // `fetched` is the delivery cursor; `redeliver` bypasses it AND leaves it alone, so a
          // re-read after a context compaction cannot swallow the reader's marks for the next turn.
          if (!redo && s.fetched === true) continue
          wants[s.id] = true
          if (!redo && s.fetched !== true) { consume[s.id] = true; count++ }
        }
        const view = viewSelections()
        if (count > 0) {
          // Only the marks this call actually returned are consumed: asking for agent marks must
          // never silently swallow the reader's own unread highlights.
          S.selections = S.selections.map(function (s) { return consume[s.id] === true ? Object.assign({}, s, { fetched: true, fetchedAt: isoNow() }) : s })
          S.revision += 1
          try { await persistState() } catch (err) { fail('写入选中记录', err) }
        }
        return view.filter(function (s) { return wants[s.id] === true })
        })
        })
      },
    }))
    registerToolLocked(defineTool({
      name: 'note_status',
      description: '一次调用判断「要不要读点东西」（≤0.4 KB）：rev / git / 行数 / 标记数 + 自你上次读取以来的改动量。没有改动就不要读正文。',
      parameters: { note: { type: 'string', description: '看哪一份笔记（默认当前打开的）' } },
      output: {
        schema: {
          type: 'object', additionalProperties: false,
          properties: { note: { type: 'string', required: true }, rev: { type: 'integer', required: true }, git: { type: 'string', required: true }, dirty: { type: 'boolean', required: true }, lines: { type: 'integer', required: true }, chars: { type: 'integer', required: true }, contentSha: { type: 'string', required: true }, marks: { type: 'integer', required: true }, baseRev: { type: 'integer', required: true }, baseAt: { type: 'string', required: true }, unread: { type: 'string', required: true }, changed: { type: 'boolean', required: true } },
        },
        render: function (a, v) {
          // The tail must match the state. With NO baseline there is nothing to compare against, and
          // "没有改动，不必读正文" is the exact opposite of the truth: the honest answer is "read once to
          // establish the baseline". (Found by the live acceptance run: the unit suite only exercised
          // the with-baseline paths.)
          const tail = v.baseRev === 0
            ? '\n→ 还没有基线：先 note_read({mode:"auto"}) 读一次（首次读取会给全文并记下基线）'
            : (v.changed ? '\n→ 用 note_diff({format:"hunks"}) 看具体改动；需要全文才 note_read({mode:"full"})' : '\n（没有改动，不必读正文）')
          return [{ type: 'text', text: '[note=' + v.note + ' rev=' + v.rev + ' git=' + v.git + ' 行=' + v.lines + ' 标记=' + v.marks + ']\n自你上次读取（rev ' + v.baseRev + '，' + v.baseAt + '）：' + v.unread + tail }]
        },
      },
      async execute(args, exec) {
        await enterFromTool('note_status', exec)
        return await useNote(args && args.note, async function () {
          const base = lastReadOf()
          const u = await unreadOf()
          return {
            note: activeNote || '', rev: S.revision, git: S.commitHash || 'none', dirty: S.commitHash === '',
            lines: textLines(S.text).length, chars: S.text.length, contentSha: contentHashOf(S.text), marks: S.selections.length,
            baseRev: base ? base.rev : 0, baseAt: base ? agoText(base.at) : '无基线',
            unread: u.known === true && u.changed !== true ? '无改动' : deltaText(u), changed: u.known === true && u.changed === true,
          }
        })
      },
    }))
    registerToolLocked(defineTool({
      name: 'note_diff',
      description: '只看「相对你上次读到改了什么」——不需要读全文、不需要跑 git。默认 summary（数字）；format:"hunks" 给带上下文的改动块；format:"marks" 只给标记增删。since 省略即用你上次读取的 rev。',
      parameters: {
        format: { type: 'string', enum: ['summary', 'hunks', 'marks'], description: '默认 summary。' },
        context: { type: 'integer', description: 'hunks 模式下每个改动块上下各保留几行，默认 2。' },
        since: { type: 'integer', description: '基线 rev（省略 = 你上次读到的 rev）。' },
        note: { type: 'string', description: '看哪一份笔记（默认当前打开的）' },
      },
      output: {
        schema: {
          type: 'object', additionalProperties: false,
          properties: { ok: { type: 'boolean', required: true }, baseRev: { type: 'integer', required: true }, rev: { type: 'integer', required: true }, changed: { type: 'boolean', required: true }, linesAdded: { type: 'integer', required: true }, linesRemoved: { type: 'integer', required: true }, charsAdded: { type: 'integer', required: true }, charsRemoved: { type: 'integer', required: true }, marksAdded: { type: 'integer', required: true }, marksRemoved: { type: 'integer', required: true }, text: { type: 'string', required: true }, error: { type: 'string', required: true } },
        },
        render: function (a, v) { return [{ type: 'text', text: v.ok ? v.text : ('失败: ' + v.error) }] },
      },
      async execute(args, exec) {
        await enterFromTool('note_diff', exec)
        return await useNote(args && args.note, async function () {
          const a = args || {}
          const base = lastReadOf()
          const fmt = a.format === 'hunks' || a.format === 'marks' ? a.format : 'summary'
          if (!base) {
            return { ok: true, baseRev: 0, rev: S.revision, changed: true, linesAdded: 0, linesRemoved: 0, charsAdded: 0, charsRemoved: 0, marksAdded: 0, marksRemoved: 0, text: statusHead() + '\n还没有基线（你还没用 note_read 读过这份笔记）——「相对上次」无从谈起；请直接用 note_read 读一次（默认 mode:auto 会给全文并记下基线）。', error: '' }
          }
          if (a.since !== undefined && intOr(a.since, -1) >= 0 && intOr(a.since, -1) !== base.rev) {
            return { ok: false, baseRev: base.rev, rev: S.revision, changed: false, linesAdded: 0, linesRemoved: 0, charsAdded: 0, charsRemoved: 0, marksAdded: 0, marksRemoved: 0, text: '', error: '只保留了一份基线（rev ' + base.rev + '），无法与 rev ' + intOr(a.since, -1) + ' 比较；省略 since 即用基线。' }
          }
          const u = await unreadOf({ hunks: fmt === 'hunks', context: a.context })
          if (!u.known) return { ok: true, baseRev: base.rev, rev: S.revision, changed: true, linesAdded: 0, linesRemoved: 0, charsAdded: 0, charsRemoved: 0, marksAdded: 0, marksRemoved: 0, text: '基线的快照文件不见了（可能被清理）——用 note_read 重读一次即可重建。', error: '' }
          if (fmt === 'marks') {
            const text = '[note=' + (activeNote || '') + ' rev=' + S.revision + '] 标记自 rev ' + u.rev + ' 起：+' + u.marksAdded + '/-' + u.marksRemoved
            return { ok: true, baseRev: u.rev, rev: S.revision, changed: u.changed === true, linesAdded: u.linesAdded, linesRemoved: u.linesRemoved, charsAdded: u.charsAdded, charsRemoved: u.charsRemoved, marksAdded: u.marksAdded, marksRemoved: u.marksRemoved, text: text, error: '' }
          }
          if (fmt === 'hunks') {
            const rows = hunksRender(u.hunks || [], 60)
            const head = '[note=' + (activeNote || '') + ' rev=' + u.rev + '→' + S.revision + '] ' + deltaText(u)
            const tail = u.diff && u.diff.tooBig ? '\n(改动块太大，只给统计：整块替换)' : ''
            const text = (u.changed ? head + '\n' + (rows || '(没有可展示的行级改动)') + tail : head + '\n（没有改动）')
            return { ok: true, baseRev: u.rev, rev: S.revision, changed: u.changed === true, linesAdded: u.linesAdded, linesRemoved: u.linesRemoved, charsAdded: u.charsAdded, charsRemoved: u.charsRemoved, marksAdded: u.marksAdded, marksRemoved: u.marksRemoved, text: text, error: '' }
          }
          const text = u.changed ? ('[note=' + (activeNote || '') + ' rev=' + u.rev + '→' + S.revision + '] ' + deltaText(u) + '\n→ format:"hunks" 看具体改动') : ('[note=' + (activeNote || '') + ' rev=' + S.revision + '] 自 rev ' + u.rev + ' 以来无改动')
          return { ok: true, baseRev: u.rev, rev: S.revision, changed: u.changed === true, linesAdded: u.linesAdded, linesRemoved: u.linesRemoved, charsAdded: u.charsAdded, charsRemoved: u.charsRemoved, marksAdded: u.marksAdded, marksRemoved: u.marksRemoved, text: text, error: '' }
        })
      },
    }))
    registerToolLocked(defineTool({
      name: 'note_read',
      description: '读取笔记正文(磁盘 note.md 的实时内容)，带行号。默认 mode:auto —— 自你上次读取没有改动就只回「无改动」（不吐全文）；首次读取或明确 mode:"full" 才给全文。要定位某段先用 note_find 再 mode:"window" 开窗。',
      parameters: { mode: { type: 'string', enum: ['auto', 'window', 'full'], description: 'auto（默认）：无改动只回一行，首次读取给全文；window：用 fromLine/toLine 开窗；full：明确要全文。' }, withLineNumbers: { type: 'boolean', description: '是否在正文前加行号，默认 true。' }, note: { type: 'string', description: '要看哪一份笔记（默认当前打开的；指定别的名字不会切换卡片）' }, fromLine: { type: 'integer', description: '只读这一段的第一行(1 基，含)。省略则从头。' }, toLine: { type: 'integer', description: '读到这一行(1 基，含)。省略则到底。' }, padding: { type: 'integer', description: '上下各多读几行，默认 0。' }, detail: { type: 'string', enum: ['none', 'brief', 'full'], description: '标记概览的详细度：默认 none（不附标记，需要时用 note_mark_list）；brief 附简要列表；full 附备注与原文。' } },
      output: {
        schema: {
          type: 'object', additionalProperties: false,
          properties: { path: { type: 'string', required: true }, kind: { type: 'string', required: true }, lineCount: { type: 'integer', required: true }, fromLine: { type: 'integer', required: true }, toLine: { type: 'integer', required: true }, revision: { type: 'integer', required: true }, baseRev: { type: 'integer', required: true }, unread: { type: 'string', required: true }, marks: { type: 'string', required: true }, text: { type: 'string', required: true }, viewLine: { type: 'integer', required: true } },
        },
        render: function (a, v) {
          const where = v.viewLine ? ('\n用户当前读到: 第 ' + v.viewLine + ' 行') : ''
          const marks = v.marks ? ('\n\n--- 标记概览 ---\n' + v.marks) : ''
          if (v.kind === 'auto-same') {
            return [{ type: 'text', text: '[note=' + (v.path || '').split('/').slice(-2)[0] + ' rev=' + v.revision + '] 自你上次读取（rev ' + v.baseRev + '）以来无改动，共 ' + v.lineCount + ' 行' + where + '\n（没有读正文。需要全文请 mode:"full"；要看历史改动用 note_diff）' }]
          }
          if (v.kind === 'auto-changed') {
            const tail = v.text ? ('\n' + v.text) : ''
            return [{ type: 'text', text: 'mode=auto(只给改动，未给全文): 自 rev ' + v.baseRev + ' 起 ' + v.unread + tail + '\n→ 需要完整正文请 mode:"full"；要看完整改动块用 note_diff({format:"hunks"})' }]
          }
          ...
          const first = v.kind === 'auto-first' ? '（首次读取：这是全文，已记下基线）' : ''
          return [{ type: 'text', text: '笔记文件: ' + v.path + (range ? range : '') + ' (共 ' + v.lineCount + ' 行)' + first + where + '\n\n' + numbered + marks }]
        },
      },
      async execute(args, exec) {
        await enterFromTool('note_read', exec)
        return await useNote(args && args.note, async function () {
        // NOT markTouched(): reading is not changing. Besides flagging the note as dirty, it bumped
        // `rev` on every read, which is what made that number stop meaning "the note changed".
        const a = args || {}
        const all = linesOf(S.text)
        const pad = Math.max(0, intOr(a.padding, 0))
        const mode = a.mode === 'full' || a.mode === 'window' ? a.mode : ((a.fromLine !== undefined || a.toLine !== undefined) ? 'window' : 'auto')
        const detailMode = a.detail === 'full' || a.detail === 'brief' ? a.detail : 'none'
        const marksOf = function () { return detailMode === 'none' ? '' : selRender(viewSelections(), { detail: detailMode === 'full' ? 'full' : 'brief' }) }
        // A WINDOW read deliberately does not move the baseline: the agent has not seen the rest,
        // and pretending otherwise would silence the unread signal for the part it never read.
        if (mode === 'full') {
          await setLastRead(S.text)
          return { path: paths().note, kind: 'full', lineCount: all.length, fromLine: 1, toLine: all.length, revision: S.revision, baseRev: lastReadOf() ? lastReadOf().rev : 0, unread: '', marks: marksOf(), text: S.text, viewLine: S.view && Number.isFinite(Number(S.view.line)) ? Math.max(1, Math.round(Number(S.view.line))) : 0 }
        }
        if (mode === 'window') {
          let from = Math.max(1, intOr(a.fromLine, 1) - pad)
          let to = Math.min(all.length, intOr(a.toLine, all.length) + pad)
          if (to < from) to = from
          return { path: paths().note, kind: 'window', lineCount: all.length, fromLine: from, toLine: to, revision: S.revision, baseRev: lastReadOf() ? lastReadOf().rev : 0, unread: '', marks: marksOf(), text: all.slice(from - 1, to).join('\n'), viewLine: S.view && Number.isFinite(Number(S.view.line)) ? Math.max(1, Math.round(Number(S.view.line))) : 0 }
        }
        const u = await unreadOf({ hunks: true, context: 1, maxHunks: 2 })
        if (u.known !== true) {
          await setLastRead(S.text)
          return { path: paths().note, kind: 'auto-first', lineCount: all.length, fromLine: 1, toLine: all.length, revision: S.revision, baseRev: 0, unread: '', marks: marksOf(), text: S.text, viewLine: S.view && Number.isFinite(Number(S.view.line)) ? Math.max(1, Math.round(Number(S.view.line))) : 0 }
        }
        if (u.changed !== true) {
          return { path: paths().note, kind: 'auto-same', lineCount: all.length, fromLine: 1, toLine: 1, revision: S.revision, baseRev: u.rev, unread: '无改动', marks: '', text: '', viewLine: S.view && Number.isFinite(Number(S.view.line)) ? Math.max(1, Math.round(Number(S.view.line))) : 0 }
        }
        const rows = hunksRender(u.hunks || [], 24)
        return { path: paths().note, kind: 'auto-changed', lineCount: all.length, fromLine: 1, toLine: 1, revision: S.revision, baseRev: u.rev, unread: deltaText(u), marks: '', text: rows, viewLine: S.view && Number.isFinite(Number(S.view.line)) ? Math.max(1, Math.round(Number(S.view.line))) : 0 }
        })
      },
    }))
    registerToolLocked(defineTool({
      name: 'note_diag',
      description: '诊断笔记卡片的运行环境（只读）：落盘路径、所属会话、git 状态、rev/行数/内容指纹，以及自你上次读取的增量——一次调用就能判断「要不要读点东西」。',
      parameters: {},
      output: {
        schema: {
          type: 'object', additionalProperties: false,
          properties: { path: { type: 'string', required: true }, baseFrom: { type: 'string', required: true }, confirmed: { type: 'boolean', required: true }, touched: { type: 'boolean', required: true }, sessionId: { type: 'string', required: true }, gitReady: { type: 'boolean', required: true }, gitTrace: { type: 'string', required: true }, error: { type: 'string', required: true }, note: { type: 'string', required: true }, rev: { type: 'integer', required: true }, git: { type: 'string', required: true }, dirty: { type: 'boolean', required: true }, lines: { type: 'integer', required: true }, chars: { type: 'integer', required: true }, contentSha: { type: 'string', required: true }, marks: { type: 'integer', required: true }, marksDirty: { type: 'boolean', required: true }, unread: { type: 'string', required: true }, baseRev: { type: 'integer', required: true }, baseAt: { type: 'string', required: true }, heapMB: { type: 'integer', required: true }, rssMB: { type: 'integer', required: true }, sessions: { type: 'integer', required: true }, noteTextKB: { type: 'integer', required: true }, facts: { type: 'integer', required: true } },
        },
        render: function (a, v) {
          return [{ type: 'text', text: 'path=' + v.path + '\nbaseFrom=' + v.baseFrom + ' confirmed=' + v.confirmed + ' touched=' + v.touched + '\nownerSessionId=' + v.sessionId + '\ngitReady=' + v.gitReady + ' error=' + v.error + '\n[note=' + v.note + ' rev=' + v.rev + ' git=' + v.git + ' 行=' + v.lines + ' 标记=' + v.marks + ']' + (v.marksDirty ? ' （有未提交改动）' : '') + '\n自你上次读取（rev ' + v.baseRev + '，' + v.baseAt + '）：' + v.unread + '\n进程内存: heap ' + v.heapMB + ' MB / rss ' + v.rssMB + ' MB；本插件自己握着: ' + v.sessions + ' 个会话的 store、笔记正文合计 ' + v.noteTextKB + ' KB、' + v.facts + ' 条笔记事实缓存' }]
        },
      },
      async execute(args, exec) {
        await enterFromTool('note_diag', exec, { allowEmpty: true })
        // NOT markTouched(): this tool is documented as read-only, and it was flagging the note as
        // "changed" by the mere act of looking at it — the live acceptance run called note_commit and
        // then note_diag, and the answer claimed there were uncommitted changes. It also bumped `rev`,
        // which made that number useless as a change counter. (The card's "未提交" hint is about the
        // marks, and only a tool that actually changes them should raise it.)
        const base = lastReadOf()
        const u = await unreadOf()
        // What THIS plugin retains, next to what the process as a whole uses. A heap-out-of-memory
        // report needs exactly this split: the plugin holds one note body per live session plus a few
        // small maps, so if the numbers below are a few MB while the heap is gigabytes, the memory is
        // somewhere else (the subagent/session machinery, for instance).
        let textBytes = 0
        let selCount = 0
        try {
          const each = stores.values()
          for (let it = each.next(); !it.done; it = each.next()) {
            const st = it.value
            if (st && st.S) {
              if (typeof st.S.text === 'string') textBytes += st.S.text.length
              if (Array.isArray(st.S.selections)) selCount += st.S.selections.length
            }
          }
        } catch (err) { }
        let mem = null
        try { if (typeof process !== 'undefined' && typeof process.memoryUsage === 'function') mem = process.memoryUsage() } catch (err) { mem = null }
        return {
          path: paths().note, baseFrom: baseFrom, confirmed: confirmed, touched: S.touched === true, sessionId: sessionId,
          gitReady: S.gitReady, gitTrace: S.gitTrace, error: S.error,
          note: activeNote || '', rev: S.revision, git: S.commitHash || 'none', dirty: S.commitHash === '',
          lines: textLines(S.text).length, chars: S.text.length, contentSha: contentHashOf(S.text),
          marks: S.selections.length, marksDirty: S.touched === true,
          unread: u.known === true && u.changed !== true ? '无改动' : deltaText(u), baseRev: base ? base.rev : 0, baseAt: base ? agoText(base.at) : '无基线',
          heapMB: mem ? Math.round(mem.heapUsed / 1048576) : 0,
          rssMB: mem ? Math.round(mem.rss / 1048576) : 0,
          sessions: stores.size,
          noteTextKB: Math.round(textBytes / 1024),
          facts: noteFacts.size,
        }
      },
    }))
    registerToolLocked(defineTool({
    ...
    registerToolLocked(defineTool({
      name: 'note_commit',
      description: '把笔记卡片当前内容 git 提交一次（用户点保存按钮做的是同一件事）。reason 是提交原因，写进提交信息便于回溯 —— 长任务里记检查点用它（旧的 note_checkpoint 是同一件事，已并入这里）。',
      parameters: { message: { type: 'string', description: '提交信息，省略则按 delta 自动生成。' }, reason: { type: 'string', description: '提交原因/检查点说明；等价于 message（两者都给时 message 优先）。' } },
      output: {
      ...
      async execute(args, exec) {
        ...
        markTouched()
        const a = args || {}
        const asked = a.message ? String(a.message) : (a.reason ? String(a.reason) : '')
        const r = await commit(asked)
    ...
    registerToolLocked(defineTool({
      ...
      output: {
        ...
        render: function (a, v) {
          ...
          const un = v.unread ? ('\n当前这篇《' + v.active + '》自你上次读取：' + v.unread) : ''
          return [{ type: 'text', text: '会话 ' + v.notesDir.split('/').slice(-1)[0] + ' 的笔记（' + (v.notes || []).length + ' 份）' + (v.active ? '，当前打开: ' + v.active : '，当前没有打开任何笔记') + '\n' + (rows.length ? rows.join('\n') : '(还没有笔记，用 note_create 新建)') + un }]
        },
      },
      async execute(args, exec) {
        await enterFromTool('note_list', exec, { allowEmpty: true })
        const notes = await notesView()
        // Only the note on screen gets an unread number: a diff per note would mean reading and
        // comparing every note on every list call (160 notes is a real session).
        const u = activeNote ? await unreadOf() : { known: false }
        const unreadText = u.known === true ? (u.changed === true ? deltaText(u) : '无改动') : ''
        return { notesDir: sessionRoot(), active: activeNote, notes: notes, unread: unreadText }
      },
    }))
    registerToolLocked(defineTool({
      name: 'note_create',
      description: '在本会话新建一份笔记并默认打开它。支持三种来源：空笔记(默认)、从工作区里的文件导入(from=<路径>)、或直接给内容(text)。建好后立刻为该笔记建仓并提交一次，gitReady 回报结果（批量文件夹导入仍按需延迟建仓）。',
      parameters: {
      ...
      output: {
        schema: { type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean', required: true }, name: { type: 'string', required: true }, active: { type: 'string', required: true }, source: { type: 'string', required: true }, lineCount: { type: 'integer', required: true }, dir: { type: 'string', required: true }, gitReady: { type: 'boolean', required: true }, error: { type: 'string', required: true } } },
        render: function (a, v) {
          const git = v.gitReady ? 'git 已就绪' : (v.active === v.name ? 'git 未就绪' : 'git 待首次需要时创建')
          return [{ type: 'text', text: v.ok ? ('已新建笔记《' + v.name + '》(' + v.source + ', ' + v.lineCount + ' 行, ' + git + ')' + (v.active === v.name ? '，并已打开' : '') + '\n目录: ' + v.dir) : ('新建失败: ' + v.error) }]
        },
      },
      async execute(args, exec) {
        ...
        return await withNoteLock(noteLockKey(), async function () {
          ...
          // P1-7: this tool creates ONE note the caller is about to work in, so its repository is
          // built right here and the answer carries `gitReady`. Deferring it made a successful
          // note_create look broken: `note_diag` answered gitReady:false until something else needed
          // history. Every other creator (the card's 新建 RPC, the 156-document folder import) keeps
          // the lazy path — eager repositories are what made a bulk import unusable.
          if (r.ok === true && r.active === r.name && shell !== undefined && canWrite() && S.gitReady !== true) {
            await ensureGit()
            await ensureViewIgnored()
          }
          ...
          return Object.assign({ ok: false, name: String((args && args.name) || ''), active: activeNote, source: '', lineCount: 0, dir: '', gitReady: false, error: '' }, r, { gitReady: r.ok === true && r.active === r.name ? S.gitReady === true : false })
        })
      },
    }))
    registerToolLocked(defineTool({
    ...
    registerToolLocked(defineTool({
      name: 'note_clear_keep_history',
      description: '清空当前笔记的正文，但**保留它的 git 历史**（清空前会自动提交一次快照，清空后再提交一次）。想彻底删除整份笔记请用 note_delete_forever。',
      ...
      async execute(args, exec) {
        await enterFromTool('note_clear_keep_history', exec)
        return await withNoteLock(noteLockKey(), async function () {
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        await enterFromTool('note_delete_forever', exec, { allowEmpty: true })
        return await withNoteLock(noteLockKey(), async function () {
    ...
    /**
     * `note_append` and `note_replace`: one concept per name (§P1-9).
     *
     * The old single `note_import` carried a `mode: append|replace` switch — the naming review's
     * "一名两物": the same word meant "add to the body" here and "build notes from a whole folder"
     * in `note_folder_import`. Two thin tools over one implementation say what they do.
     */
    const APPEND_PARAMS = { from: { type: 'string', description: '源文件路径（与 text 二选一）' }, text: { type: 'string', description: '直接给内容（与 from 二选一）' } }
    const APPEND_SCHEMA = { type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean', required: true }, name: { type: 'string', required: true }, mode: { type: 'string', required: true }, source: { type: 'string', required: true }, lineCount: { type: 'integer', required: true }, error: { type: 'string', required: true } } }
    registerToolLocked(defineTool({
      name: 'note_append',
      description: '把外部内容追加到当前笔记末尾：from=<文件路径> 或 text=<直接内容>。',
      parameters: APPEND_PARAMS,
      output: {
        schema: APPEND_SCHEMA,
        render: function (a, v) { return [{ type: 'text', text: v.ok ? ('已追加到《' + v.name + '》(来源 ' + v.source + ')，现在 ' + v.lineCount + ' 行') : ('失败: ' + v.error) }] },
      },
      async execute(args, exec) {
        await enterFromTool('note_append', exec)
        return await withNoteLock(noteLockKey(), async function () {
          const r = await importIntoActiveNote(Object.assign({}, args || {}, { mode: 'append' }))
    ...
    registerToolLocked(defineTool({
      name: 'note_replace',
      description: '用外部内容覆盖当前笔记正文：from=<文件路径> 或 text=<直接内容>。覆盖会丢掉现有正文（已有标记会自动重新锚定）。',
      parameters: APPEND_PARAMS,
      output: {
        schema: APPEND_SCHEMA,
        render: function (a, v) { return [{ type: 'text', text: v.ok ? ('已覆盖《' + v.name + '》(来源 ' + v.source + ')，现在 ' + v.lineCount + ' 行') : ('失败: ' + v.error) }] },
      },
      async execute(args, exec) {
        await enterFromTool('note_replace', exec)
        return await withNoteLock(noteLockKey(), async function () {
          const r = await importIntoActiveNote(Object.assign({}, args || {}, { mode: 'replace' }))
          return Object.assign({ name: activeNote, mode: 'replace', source: '', lineCount: 0, error: '' }, r)
        })
      },
    }))
    registerToolLocked(defineTool({
    ...
    // 「确认变动」 on a stale mark: the range is what the reader sees, the text under it is the new
    // truth. Answering through a handler (rather than letting the card write the record) is what
    // keeps every mark mutation inside the store lock and in one place.
    handleLocked('confirmSelection', async function (args) {
      if (!bindSession(args && args.sessionId)) return notMine('confirmSelection')
      confirmed = true
      await ensureLoaded()
      return await withNoteLock(noteLockKey(), function () { return confirmStaleSelection(args && args.id) })
    })
    ...
    handleLocked('openMirrorDoc', async function (args) {
      ...
      // A jump inside THIS note needs nothing but this note: no asset root, no mirror, no file.
      if (rawPath === '') {
    ...
    async function flushAfterWrite() {
      ...
      try { await persistState() } catch (err) { fail('写入选中记录', err) }
      // The text changed: keep the summary the prompt variable prints in step with it.
      await refreshUnread()
      return true
    }
    const PATCH_SCHEMA = {
      type: 'object', additionalProperties: false,
      properties: { ok: { type: 'boolean', required: true }, edits: { type: 'integer', required: true }, changed: { type: 'boolean', required: true }, removed: { type: 'integer', required: true }, inserted: { type: 'integer', required: true }, revision: { type: 'integer', required: true }, lineCount: { type: 'integer', required: true }, marks: { type: 'integer', required: true }, error: { type: 'string', required: true } },
    }
    function patchRender(a, v) {
      if (!v.ok) return [{ type: 'text', text: '失败: ' + v.error }]
      const n = intOr(v.edits, 1)
      if (!v.changed) return [{ type: 'text', text: n > 1 ? ('无变化（' + n + ' 处区间的内容都与替换文本相同）') : '无变化（区间内容与替换文本相同）' }]
      return [{ type: 'text', text: '已改写' + (n > 1 ? (' ' + n + ' 处') : '') + ': -' + v.removed + ' +' + v.inserted + ' 字符，现 ' + v.lineCount + ' 行，标记 ' + v.marks + ' 条 (rev ' + v.revision + ')' }]
    }
    // The same four coordinates, deliberately NOT required: note_patch takes either one range or an
    // `edits` array, so neither form can be mandatory (§P1-9 merged note_patch_many into it).
    const PATCH_RANGE = {
      startLine: { type: 'integer', description: '起始行（1 基）' },
      startCol: { type: 'integer', description: '起始列（0 基）' },
      endLine: { type: 'integer', description: '结束行（1 基，含）' },
      endCol: { type: 'integer', description: '结束列（0 基，不含）' },
    }
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_mark_add', exec); markTouched()
        const a = args || {}
        const r = await addSelection(Object.assign({}, a, { author: 'agent', color: colorOf(a.color), italic: a.italic === true || styleWants(a.style).italic, underline: a.underline === true || styleWants(a.style).underline }))
        return { ok: r.ok === true, id: r.ok ? r.id : '', revision: S.revision, marks: S.selections.length, reason: r.ok ? '' : String(r.reason || '') }
        })
      },
    }))
    registerToolLocked(defineTool({
      name: 'note_mark_remove',
      description: '删除一条标记（按 id，id 来自 note_mark_list）。',
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_mark_remove', exec); markTouched()
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_mark_clear', exec); markTouched()
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_mark_color', exec); markTouched()
        const a = args || {}
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_mark_style', exec); markTouched()
        const a = args || {}
        const patch = {}
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
          await enterFromTool('note_mark_remark', exec); markTouched()
          const a = args || {}
    ...
    registerToolLocked(defineTool({
      name: 'note_scroll_to',
      description: '让卡片跳到笔记的某个位置（和用户自己滚动到那一样，只是由 agent 发起）。可以给 line，也可以给 markId（标记 id，来自 note_mark_list）直接跳到那条标记。可选 note 参数先切到那份笔记。用于"看这里"、"我把改好的地方给你翻出来"。',
      parameters: {
      ...
      async execute(args, exec) {
        await enterFromTool('note_scroll_to', exec)
        return await withNoteLock(noteLockKey(), async function () {
          const a = args || {}
          if (a.note) {
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        await enterFromTool('note_card_panel', exec, { allowEmpty: true })
        return await withNoteLock(noteLockKey(), async function () {
          const a = args || {}
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        await enterFromTool('note_mark_lists', exec, { allowEmpty: true })
        return await withNoteLock(noteLockKey(), async function () {
          const a = args || {}
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        await enterFromTool('note_card_ui', exec, { allowEmpty: true })
        return await withNoteLock(noteLockKey(), async function () {
          const a = args || {}
    ...
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
        await enterFromTool('note_folder_import', exec, { allowEmpty: true })
        const a = args || {}
        if (a.list === true) {
    ...
    registerToolLocked(defineTool({
      name: 'note_patch',
      description: '按区间精确改写笔记的一小段，无需输出全文——这是首选的写入方式（省上下文）。行 1 基、列 0 基、列以该行原文为准，列到行尾可用 endLine 下一行:0。多处一起改写给 edits:[{startLine,startCol,endLine,endCol,text},…]（内部从后往前应用，不会互相错位）。写入后会自动重新锚定已有标记。',
      parameters: {
        startLine: PATCH_RANGE.startLine, startCol: PATCH_RANGE.startCol, endLine: PATCH_RANGE.endLine, endCol: PATCH_RANGE.endCol,
        text: { type: 'string', description: '替换文本；空串表示删除该区间。' },
        edits: {
          type: 'array', description: '一次改多处：每项含 startLine/startCol/endLine/endCol/text，按你给出的顺序即可。给了 edits 就不必给单区间四个参数。',
      ...
      async execute(args, exec) {
        ...
        const a = args || {}
        const list = Array.isArray(a.edits) ? a.edits.slice() : []
        if (list.length) {
          // Many ranges in one call: applied back-to-front so an earlier edit cannot shift a later
          // one's coordinates. This was `note_patch_many` — the same operation, so §P1-9 folds it
          // into this tool as `edits` instead of paying for a second description every turn.
          ...
          for (let i = 0; i < withIdx.length; i++) {
            const rMany = applyPatch(withIdx[i].e)
            applied += 1
            if (rMany.changed) { changed = true; removed += rMany.removed; inserted += rMany.inserted }
          }
          const wroteMany = changed ? await flushAfterWrite() : true
          return { ok: wroteMany, edits: applied, changed: changed, removed: removed, inserted: inserted, revision: S.revision, lineCount: linesOf(S.text).length, marks: S.selections.length, error: wroteMany ? '' : S.error }
        }
        const r = applyPatch(a)
        if (!r.changed) return { ok: true, edits: 1, changed: false, removed: 0, inserted: 0, revision: S.revision, lineCount: linesOf(S.text).length, marks: S.selections.length, error: '' }
        const wrote = await flushAfterWrite()
        return { ok: wrote, edits: 1, changed: true, removed: r.removed, inserted: r.inserted, revision: S.revision, lineCount: linesOf(S.text).length, marks: S.selections.length, error: wrote ? '' : S.error }
        })
      },
    }))
    registerToolLocked(defineTool({
      ...
      async execute(args, exec) {
        await enterFromTool('note_find', exec)
        return await useNote(args && args.note, async function () {
        const a = args || {}
        ...
        ctx.effect(function () {
          return systemPrompt.section({
            ...
            text: [
              ...
              '工作方式(仅当你归属于这张卡片时适用):',
              '- 【省上下文铁律 · 读笔记前必做】',
              '  ① 先 `note_status`（≤0.4 KB）看 rev/行数/标记数 + 自你上次读取以来的改动量：**没有改动就不要读正文**；',
              '  ② 有改动优先 `note_read({mode:"auto"})`（默认，按基线只给增量）或 `note_diff({format:"hunks"})`，不要整篇重读；',
              '  ③ 只有确实需要通读时才 `note_read({mode:"full"})`，并在回复里说明理由（它会把基线推进到当前版本）；',
              '  ④ 只想知道"关于某话题的那段"用 `note_find` 定位 + `note_read({mode:"window", fromLine, toLine})` 开窗（窗口读取**不会**推进基线，其余部分对模型仍然是未读）；',
              '  ⑤ 不要绕过插件用通用读文件工具直接读 note.md（那会丢掉行号对齐、增量基线与标记锚定）。',
              '- 回答用户前先 `note_mark_new` 取用户新标记：它给出用户自上次取用后新划的重点（默认只给用户划的，agent 自己加的标记不会混进来；id/行范围/颜色 + 前 30 字预览，已取用的不重复给）。用户标记往往就是"这里我不懂/我要你展开"。上下文被压缩后想再看一遍已取用的，传 `redeliver:true`（不消耗取用状态）。',
              '- 标记默认 **brief**：`note_mark_list` 每条只给 id/行范围/颜色/样式/状态 + 前 30 字；要看某几条的备注与原文时用 `note_mark_list({ids:["sel-3"], detail:"full"})`，不要为了两条把全部标记的全文拉回来。',
              ...
              '- 改一小段就用 `note_patch`（按区间替换，不必输出全文）；多处一起改就给它的 `edits` 数组（内部从后往前应用）。',
              '- 要定位"关于某话题的那段"用 `note_find`，拿到行号列号后直接 `note_patch`，不要先读全文；只读窗口用 `note_read({fromLine,toLine,padding})`。',
              '- 你也能管理标记：`note_mark_add`（新建，可指定颜色与样式）/ `note_mark_remove` / `note_mark_color` / `note_mark_style` / `note_mark_clear`。',
              '- 颜色即意图：yellow=重点、pink=疑问（要我展开）、green=已确认/已处理、black=遮盖（这段不要引用也不要复述）、none=不铺底色（只保留样式）。处理完一条标记后，可用 `note_mark_color` 把它转成 green 作为闭环信号。',
              '- 样式是**两个独立开关**：`note_mark_style({id, italic:true})` 只改斜体、`underline:true` 只改下划线，两者可以同时开，并且**和任何颜色叠加**（例如绿底+斜体+下划线）。它们直接改文字本身、不抢注意力。新建时也能一起给：`note_mark_add({…, color:"pink", italic:true, underline:true})`。',
              '- 自定义标记列表：`note_mark_lists`（action = list/create/rename/delete/add/remove）能把任意笔记里的任意标记收进一个命名列表（例如"面试要背的"），写的是同一份会话数据，用户在卡片里点的「添加到」也是它。',
              '- 卡片界面也能由你驱动：`note_card_ui`（open/close/tab/float/dock/summon/focus/card/refresh —— 打开或关闭标记列表、切视图、拖出成独立小窗、改"点标记→列表"开关、把列表定位到某条标记、直接把功能卡弹在某条标记上）、`note_card_panel`（start/stop/collapse/expand/width）、`note_scroll_to`（给 line 或 markId 跳转）。命令会排队并在卡片下一次轮询时执行。',
        ...
        if (typeof systemPrompt.variable === 'function') {
          ctx.effect(function () {
            return systemPrompt.variable('dsh_window_note_scope', function (context) {
              ...
              if (st.activeNote) {
                const un = st.S && typeof st.S.unreadText === 'string' && st.S.unreadText !== '' ? (' **自你上次读取：' + st.S.unreadText + '**（要看改动用 note_diff，没改动就别读正文）。') : ''
                return '本会话的笔记空间：**已打开《' + st.activeNote + '》**（本会话共 ' + String((st.sessionNotes || []).length) + ' 份笔记，目录 ' + st.base + '/' + ROOT_DIR + '/' + NOTES_DIR + '/' + sid + '）。' + un + '下面的工作方式全部适用，包括回答前先取用户的新标记。'
              }
