...
export function apply(ctx, config) {
  ...
  void config
    ...
    registerToolLocked(defineTool({
      name: 'note_ui',
      description: '操纵卡片的标记列表界面（等价于用户点那几个按钮）：open/close（打开或关闭标记列表）、tab（切到 本笔记 / 本会话 / 某个自定义列表）、float/dock（把列表拖出成独立小窗 / 收回卡片内）、summon（"点标记→列表"开关）、focus（把列表滚到某条标记并高亮）、refresh（重新拉取）。',
      parameters: {
        action: { type: 'string', required: true, description: 'open / close / tab / float / dock / summon / focus / refresh' },
        tab: { type: 'string', description: 'action=tab：note（本笔记）/ session（本会话）/ 自定义列表名' },
        markId: { type: 'string', description: 'action=focus：要定位到的标记 id（可带 note）' },
        note: { type: 'string', description: 'action=focus：标记所在笔记（省略则当前打开的）' },
      ...
      async execute(args, exec) {
        ...
        return await withNoteLock(noteLockKey(), async function () {
          ...
          const allowed = ['open', 'close', 'tab', 'float', 'dock', 'summon', 'focus', 'refresh']
          ...
          if (act === 'focus') {
            ...
            if (id === '') return { ok: false, action: act, id: 0, error: 'action=focus 需要 markId' }
