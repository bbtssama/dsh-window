  ...
  void config
    ...
    async function countTree(dir) {
      ...
      while (stack.length) {
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          files += 1
          bytes += intOr(e && (e.size !== undefined ? e.size : e.bytes), 0)
        }
      }
    ...
    async function listMarkdownIn(dir) {
      ...
      try {
        ...
        entries = await fs.listDir(target, undefined)
      } catch (err) { return { ok: false, error: '读不到这个目录: ' + ((err && err.message) || String(err)), files: [] } }
      for (let i = 0; i < entries.length; i++) {
        const e = entries[i]
        const name = e && e.name ? String(e.name) : ''
        ...
        const isDir = e && (e.kind === 'directory' || e.type === 'directory' || e.isDirectory === true)
        if (isDir) continue
        out.push({ name: name, size: intOr(e && (e.size !== undefined ? e.size : e.bytes), 0) })
      }
    ...
    handleLocked('scanFolder', async function (args) {
      ...
      return Object.assign({ dir: dir }, r)
    })
    /**
    ...
    handleLocked('pickFolder', async function () {
      ...
      if (picker === undefined) return { ok: false, error: '这台机器没有可用的目录选择器，请手动粘贴路径' }
      try {
        const cap = typeof picker.capability === 'function' ? picker.capability() : null
        if (!cap || cap.kind !== 'native') return { ok: false, error: '当前目录选择器不支持弹窗（kind=' + String(cap && cap.kind) + '），请手动粘贴路径' }
        const picked = await cap.pick(undefined)
