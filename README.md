...
**编辑（Typora 式就地编辑）**
- **进入方式可选**：「⋯ → 设置…」里有一个开关 —— **默认"双击才编辑"**（单击正文只是单击，不会误触进编辑器）；打开「单击正文即进入编辑」后才是 Typora 那种"点一下就改"，光标落在你点的那个字符上。两种模式下，单击一段标记文字都仍然是标记功能卡；开关记在浏览器本地存储里。
- **双击任何一段都进入编辑**（不受开关影响）：段落 / 标题 / 列表项 / 引用 / 代码块，就地编辑，不再弹出单独的原码框；表格仍是整行源码框。
...
- **围栏不在编辑缓冲区里**：代码块编辑的是**块体**，那两行 ` ``` ` 不会被误删。
- 段落/标题/列表项/引用编辑时显示**原始 Markdown**（`**粗体**` 就是字面的 `**粗体**`），这正是 Typora 对"正在编辑的那个元素"的做法；表格与 Mermaid 仍然给源码框（一个是网格、一个是源码的画）。
...
- 就地编辑目前覆盖**段落 / 标题 / 列表项 / 引用 / 代码块**；表格与 Mermaid 仍然是源码框（双击打开）。
- 段落/标题等在编辑时显示的是**原始 Markdown**（不隐藏 `**`、`#`），和 Typora 隐藏标记的做法不同 —— 换来的是"渲染层的字与文本框的字逐个字符对齐"这个硬保证。
- 在被标记的那一行上，**单击**先给标记功能卡（老行为优先），要在那一行就地编辑请**双击**。
