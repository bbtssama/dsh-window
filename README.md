...
**编辑（Typora 式就地编辑）**
- **双击就地编辑**（默认）／单击就地编辑，在「⋯ → 设置…」里切换：设置卡片里的 **【双击才编辑】默认开启** —— 单击正文只是单击，不会误触进编辑器；关掉它才是"点一下就改"。
- 双击正文里的一段（段落 / 标题 / 列表项 / 引用 / 代码块）就**就地进入编辑**，光标落在你点的那个字符上；不再弹出一个单独的原码框。
- **表格单元格同样就地编辑**：双击一个格子，改的是**那个格子里的字** —— 管道符、对齐行、其他格子一个字节都不会动（`Enter` 提交、`Esc` 取消、点别处也提交）。格子在编辑时自己的字是可见的（不是透明层），所以你打的字立刻看得见。
- **代码块边打字边高亮**：编辑时读到的是**渲染层**（保持语法高亮、字体、内边距、行高完全不变），真正吃键盘的是盖在它上面、**文字透明**的 textarea —— 光标和选区在文本层，看得见的字来自渲染层。所以进编辑时**块不会跳动、不会变形**，标记底色覆盖层也照样贴在原来的位置。
- **围栏不在编辑缓冲区里**：代码块编辑的是**块体**，那两行 ` ``` ` 不会被误删；**引用里的代码块**（`> ```bash`）同样就地编辑，提交时自动把 `> ` 前缀补回去。
- 段落/标题/列表项/引用编辑时显示**原始 Markdown**（`**粗体**` 就是字面的 `**粗体**`），这正是 Typora 对"正在编辑的那个元素"的做法；Mermaid 仍然给源码框（它是源码的画）。
- `Esc` 取消（磁盘不会被碰）、`Ctrl/Cmd+Enter` 提交、**点击别处即提交**、代码块里 `Tab` 缩进。
- **不卡**：敲键盘不会触发 React 重渲染（缓冲区在 ref 里、渲染层用命令式更新、只重算改动的那一行），整卡几何重测被合并到停顿后一次。实测 120 行代码块：**每次按键中位 0.8 ms**（p90 1.0 ms，峰值 1.5 ms）。

**解析容错（真实笔记喂出来的两条）**
- **引用块里的围栏**（`> ```bash` … `> ``` `）现在解析成真正的代码块，并保持引用样式 —— 这类写法在真实笔记里很常见。
- **缩进围栏没写收尾**时按缩进结束（Typora 的行为）：两格缩进的围栏在遇到"缩进更小的非空行"时结束，不会再吞掉后面的 ```bash 及其语言标记（实测某真实笔记整段因此没渲染成代码块；收尾补齐前也能正确显示）。
...
- 就地编辑覆盖**段落 / 标题 / 列表项 / 引用 / 代码块 / 表格单元格**；Mermaid 仍然是源码框（双击打开）。
- 表格编辑是**单元格级**的：改一个格子的字，不能整行/整列插入删除（那要动结构，风险大得多）。
- 段落/标题等在编辑时显示的是**原始 Markdown**（不隐藏 `**`、`#`），和 Typora 隐藏标记的做法不同 —— 换来的是"渲染层的字与文本框的字逐个字符对齐"这个硬保证。
- 在被标记的那一行上，**单击**先给标记功能卡（老行为优先），要在那一行就地编辑请**双击**（双击始终是编辑）。
- 「双击才编辑」这个开关记在**浏览器本地存储**里（和其他卡片偏好一致），不是跟着笔记走。
