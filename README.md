...
- **递归扫描 + 手选注册**：选文件夹时**递归**扫出**所有层级**的 `.md`（实测某真实笔记文件夹扫出 **156 个，其中 154 个在子目录**），清单按缩进列出、默认全选、可「全选 / 全不选」逐个勾选要注册成笔记的。同名不同目录的文件**不会互相顶掉**：第二份自动带上父目录名（`README` / `详细笔记-README` / 再冲突加 `-2`）。
...
- **镜像里不会装着镜像**：被导入的文件夹里本身就带一个笔记空间时（内部含一个工作区，或它自己那份 `dsh-window`），那层 **`<…>/dsh-window/note`**（连它的 `_assets`）在**复制时**就被排除，已经躺在镜像里的也会被清掉（超长路径用 `\\?\` 删除）。实测：某真实镜像因此从 3542 文件 / 74.08 MB 降到 1169 文件 / 24.29 MB，而**源目录一个字节没动**。
...
- **跟着链接走**：点击正文里的**本地** `.md` 链接（例如 `[详解](知识库/Spring-IoC与Bean详解.md)`）会把它**打开成一份笔记**并共享同一个素材根 —— 于是那篇文档自己的图片、以及它指向邻居的相对链接都照常工作；再点一次就是切回那份笔记。在线链接不受影响。
  - **目标还没注册笔记？就地注册**：跟随链接时若那份文件还没有笔记，会**当场建一份**并带上素材根与来源路径（同步、图片、它自己的邻居链接立刻可用）。
  - **相对谁**：链接按**它所在的那份文件**的目录解析（Markdown 的规矩），文件里找不到时再按**文件夹根目录**试一次 —— 于是嵌套文档里的兄弟引用、以及按根目录写的目录页**都能用**。
  - **`文件.md#锚点` 直接跳到那一行**：Typora 目录页那种 `[0. 先画地图](#oop-overview)` 会打开目标笔记**并定位到对应行**。锚点既认显式 HTML 标签（`<a id="oop-overview"></a>` —— 实测某真实笔记就是这么标的，跳到了**第 27 行**），也认标题 slug（含 `-1`/`-2` 重复编号、`%E5%…` 这种百分号编码）。同一个文件里的 `#锚点` 就地在当前笔记里跳。
