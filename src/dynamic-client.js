  ...
  apply(ctx) {
    ...
    function NoteCard(props) {
      ...
      React.useEffect(function () {
        ...
        if (name === '') return
        if (navGuardRef.current === name) { navGuardRef.current = null; return }
        const nav = navRef.current
        if (nav.list[nav.at] === name) return
      ...
      function navGo(delta) {
        ...
        nav.at = at
        navGuardRef.current = nav.list[at]
        switchNote(nav.list[at])
      }
      ...
      if (menuOpen) {
        ...
        if (menuOpen.kind === 'notes') {
          ...
          items.push(mi('back', '← 后退' + (canBack ? '  →  ' + nav.list[nav.at - 1] : ''), false, function () { navGo(-1) }, canBack ? '' : 'dn-menu-item-sub', undefined, 0, true))
          items.push(mi('fwd', '→ 前进' + (canFwd ? '  →  ' + nav.list[nav.at + 1] : ''), false, function () { navGo(1) }, canFwd ? '' : 'dn-menu-item-sub', undefined, 0, true))
