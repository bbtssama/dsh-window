  ...
  apply(ctx) {
    ...
    function NoteCard(props) {
        ...
        return h('div', {
          ...
          h('div', { className: 'dn-mark-top', key: 't' }, [
            ...
            h('span', { className: 'dn-mark-acts', key: 'a' }, [
              ...
              h('button', {
                className: 'dn-mact', key: 'add', title: listName ? '从这个列表移出' : '添加到自定义列表',
                onClick: function (e) {
                  e.stopPropagation()
                  if (listName) { listRemoveMark(listName, cross ? noteName : '', m.id); return }
                  const r = e.currentTarget.getBoundingClientRect()
                  setAddFor({ markId: m.id, note: cross ? noteName : '', x: Math.round(r.left), y: Math.round(r.bottom + 6) })
                },
              }, listName ? '移出' : '添加到'),
              cross || listName ? null : h('button', {
      ...
      const marksHeadEl = h('div', {
        ...
        h('span', { className: 'dn-tabs', key: 'tabs' }, [
          ...
          ...(listData || []).map(function (l) {
            return h('button', {
              className: 'dn-tab' + (markTab === 'list:' + l.name ? ' dn-tab-on' : ''), key: 'l' + l.id,
              title: '自定义列表《' + l.name + '》：' + l.count + ' 条。右键（或长按）删掉这个列表',
              onClick: function () { setMarkTab('list:' + l.name); loadAllMarks() },
              onContextMenu: function (e) { e.preventDefault(); listDeleteRemote(l.name) },
            }, l.name + ' ' + l.count)
          }),
          h('button', {
            className: 'dn-tab dn-tab-add', key: 'new', title: '新建一个自定义列表（把任意笔记里的标记收集起来）',
            onClick: function (e) {
              e.stopPropagation()
              const r = e.currentTarget.getBoundingClientRect()
              setAddBox({ x: Math.round(r.left), y: Math.round(r.bottom + 6) })
              setNewListName('')
            },
          }, '＋'),
        ]),
      ...
      const addPanel = (addFor || addBox) ? h('div', {
        ...
        style: {
          left: Math.round(clamp((addFor || addBox).x, 6, Math.max(6, (bounds.w || 1200) - 240))) + 'px',
          top: Math.round(clamp((addFor || addBox).y, 6, Math.max(6, (bounds.h || 800) - 140))) + 'px',
        },
        onPointerDown: function (e) { e.stopPropagation() },
      }, [
      ...
      return h('div', { className: 'dn-root', ref: rootRef, style: geo.style || undefined, 'data-panel-mode': geo.mode }, [
        ...
        panelEl, foot, modalEl, remarkEl, mcardEl, addPanel,
