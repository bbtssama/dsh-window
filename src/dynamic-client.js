...
// A fenced block inside a blockquote: the quote bar stays, the code keeps its own box.
'.dn-quote-code{padding:0 0 0 10px;}',
'.dn-quote-code .dn-pre{margin:4px 0;}',
// In-place table cell editing: the rendered cell is the mirror, the input on top of it has
// transparent glyphs, so the cell keeps its style (bold, inline code, alignment) while you type.
'.dn-td,.dn-th{position:relative;}',
'.dn-cell-edit{outline:2px solid rgba(79,124,255,.55);outline-offset:-2px;border-radius:3px;}',
'.dn-cell-in{position:absolute;left:0;top:0;width:100%;height:100%;margin:0;padding:inherit;border:0;box-sizing:border-box;background:transparent;color:inherit;caret-color:var(--dsw-alias-label-primary,#1b1b1b);outline:none;font:inherit;line-height:inherit;white-space:pre;overflow:hidden;}',
// The cell's own text is only the SHAPE here (it keeps the cell from collapsing), because the
// characters the reader types are drawn by the input itself — a transparent input would make every
// new character invisible until the commit (which is exactly what the first version did).
'.dn-cell-edit .dn-cell-mirror{visibility:hidden;}',
'.dn-modal-switch{display:flex;align-items:flex-start;gap:8px;font-size:12.5px;line-height:1.5;cursor:pointer;}',
'.dn-modal-switch input{margin:2px 0 0;width:15px;height:15px;accent-color:#4f7cff;flex:0 0 auto;}',
'.dn-modal-hint{font-size:11.5px;line-height:1.6;color:#6f7680;}',
...
function parseBlocks(text) {
  ...
  while (i < ls.length) {
    ...
    if (fence) {
      ...
      const openIndent = line.length - line.replace(/^\s*/, '').length
      i++
      const body = []
      let closed = false
      while (i < ls.length) {
        const cur = ls[i]
        const f2 = /^\s*(```|~~~)\s*$/.exec(cur)
        if (f2 && f2[1] === mark) { closed = true; break }
        // An INDENTED fence that is never closed closes where its indentation ends. A real note did
        // exactly this (a two-space fence inside a list item, no closing line): the block swallowed
        // every following line, the next ```bash fence included, and the whole passage rendered as
        // one plain code block — reported as "bash 没有渲染出代码块". Typora ends such a block when
        // the indentation stops, so a non-blank line indented LESS than the opening fence closes it
        // here too, and the line itself stays for the next block.
        if (openIndent > 0) {
          const curIndent = cur.length - cur.replace(/^\s*/, '').length
          if (cur.trim() === '') {
            // A blank line only ends it when the next non-blank line is indented LESS: a blank line
            // inside an indented code block is normal (and must not split it in two).
            let j = i + 1
            while (j < ls.length && ls[j].trim() === '') j++
            if (j < ls.length) {
              const nl = ls[j]
              if (nl.length - nl.replace(/^\s*/, '').length < openIndent) break
            }
          } else if (curIndent < openIndent) break
        }
        body.push(cur); i++
      }
      if (closed) i++
    ...
    // A fenced block INSIDE a blockquote (`> ```bash` … `> ``` `) — Typora writes this whenever a
    // code block sits in a quoted passage, and until now the parser only understood a top-level
    // fence: every quoted line became a separate quote, so the fence lines showed up as literal
    // text with stray backticks (reported from a real note). The body is stored WITHOUT the quote
    // marker, the markers stay in `prefix` so the renderer and the editor can put them back.
    const qf = /^(\s*>\s?)(```|~~~)\s*(.*)$/.exec(line)
    if (qf) {
      const prefix = qf[1], mark = qf[2], lang = qf[3].trim(), start = i
      i++
      const body = []
      while (i < ls.length) {
        const q = /^(\s*>\s?)(.*)$/.exec(ls[i])
        if (!q) break
        const inner = q[2]
        const close = new RegExp('^\\s*' + mark + '\\s*$').test(inner)
        if (close) { i++; break }
        // The CONTINUATION of one body line: a quoted code line that itself starts with `>>` (a
        // blockquote inside a blockquote) keeps its extra marker in the body.
        body.push(inner)
        i++
      }
      blocks.push({ k: 'code', line: start + 1, lang: lang, body: body, endLine: start + 1 + body.length, quoted: true, prefix: prefix })
      continue
    }
...
return {
  ...
  apply(ctx) {
    ...
    function NoteCard(props) {
      ...
      const [editBlock, setEditBlock] = React.useState(null)      // Loupe shown while a selection handle is being dragged (iOS-style precise
      ...
      const dblClickOnlyRef = React.useRef(false)
      ...
      // The settings card. `双击才编辑` is ON by default: a single click in the text used to jump
      // straight into an editor, which turned every stray click into an edit (the reader asked for
      // the safe default). The switch is stored like the other card preferences.
      const [settingsOpen, setSettingsOpen] = React.useState(false)
      const [dblClickOnly, setDblClickOnly] = React.useState(function () { return readMarksPref().dblClickOnly !== false })
      // One table cell being edited in place: { line, at, len, value }.
      ...
      stRef.current = st
      // Read by the click handlers, which are plain DOM callbacks and must see the CURRENT value of
      // the switch rather than the one captured when they were created.
      dblClickOnlyRef.current = dblClickOnly
      // The cell editor travels with its own ref for the same reason the block editor does: the
      // commit is triggered by a document-level pointerdown, outside React's event tree.
      cellEditRef.current = cellEdit
      function setEditGesture(dblOnly) {
        setDblClickOnly(dblOnly)
        writeMarksPref({ dblClickOnly: dblOnly })
        notify(dblOnly ? '已改为：双击才编辑' : '已改为：单击即编辑')
      }
      ...
      // The table cell editor: focus it, and commit it when the pointer goes anywhere else (the same
      // contract the block editor has — the shell takes focus back on its own schedule, so a blur is
      // not a reliable "finished" signal).
      React.useEffect(function () {
        if (cellEdit === null) return undefined
        try { if (cellInputRef.current) cellInputRef.current.focus({ preventScroll: true }) } catch (err) { }
        const onDown = function (ev) {
          const input = cellInputRef.current
          const node = ev.target
          if (input && (node === input || (node && input.contains && input.contains(node)))) return
          if (node && node.closest && node.closest('.dn-bar, .dn-handle')) return
          commitCellEdit()
        }
        document.addEventListener('pointerdown', onDown, true)
        return function () { try { document.removeEventListener('pointerdown', onDown, true) } catch (err) { } }
      }, [cellEdit === null ? -1 : cellEdit.line * 100000 + cellEdit.at])
      ...
      /**
       * In-place TABLE CELL editing.
       *
       * A table row is one source line, so editing a cell is "replace the characters of this cell
       * inside that line" — the pipes, the padding and the alignment row are left exactly as they
       * were, which is what makes it safe to do on a real note. The cell itself is the mirror (its
       * bold / inline code / alignment stay visible) and a transparent input sits on top of it.
       */
      function startCellEdit(target, pt) {
        if (mode !== 'read') return false
        const cell = target && target.closest ? target.closest('[data-cell-at]') : null
        if (!cell) return false
        // Numbers straight off the DOM attributes (there is no shared int-or-default helper in the
        // client half — the host has one, which is what made this throw when it was first written).
        ...
        cellEditRef.current = { line: line, at: at, len: len, value: value }
      ...
      function commitCellEdit() {
        ...
        const next = src.slice(0, ce.at) + String(ce.value) + src.slice(ce.at + ce.len)
        if (next === src) return
        ...
        host.call('saveText', { text: text, baseRevision: revRef.current, sessionId: sidRef.current }).then(function (r) {
          ...
          if (r && r.ok) {
            revRef.current = r.revision
            textRef.current = text
            setSt(function (prev) { return prev ? Object.assign({}, prev, { revision: r.revision, text: text, lineCount: lines.length, selections: r.selections }) : prev })
            setToast('已保存这个单元格')
      ...
      function onBodyClick(e) {
        ...
        if (!m) {
          setMcard(null)
          // Editing is what a plain click does ONLY when the reader turned 「双击才编辑」 off; the
          // safe default is that a stray click does nothing (it used to jump into an editor).
          if (!dblClickOnlyRef.current) {
            if (startCellEdit(e.target, null)) return
            if (startInlineEdit(pointToPos(e.clientX, e.clientY))) return
          }
          return
        }
      ...
      function startInlineEdit(pt) {
        ...
        if (k === 'code' && range.block && typeof range.block.endLine === 'number' && range.block.endLine >= range.block.line + 1) {
          ...
          // A quoted fence: the buffer holds the code alone, and the `> ` markers are put back on
          // commit (otherwise editing a quoted block would silently un-quote it).
          if (range.block.quoted === true) prefix = String(range.block.prefix || '> ')
        }
        const lines = String(textRef.current || '').split(String.fromCharCode(10))
        if (from < 1 || to > lines.length || to < from) return false
        const raw = lines.slice(from - 1, to)
        const value = prefix === '' ? raw.join(String.fromCharCode(10)) : raw.map(function (l) { return l.replace(/^\s*>\s?/, '') }).join(String.fromCharCode(10))
        const eb = {
          from: from, to: to, kind: k, prefix: prefix,
      ...
      function onDoubleClick(e) {
        ...
        // A table cell edits ITSELF (that is what a table is made of), then the in-place gesture
        // applies (a code block edits its body, without the fences). A mermaid block or an image
        // keeps the source box it always had.
        if (startCellEdit(e.target, pt)) return
      ...
      function renderBlocks() {
        ...
        for (let bi = 0; bi < blocks.length; bi++) {
          ...
          if (b.k === 'code') {
            ...
            const pre = h('pre', { className: 'dn-pre' + (b.quoted ? ' dn-pre-quoted' : ''), key: 'p', 'data-line': b.quoted ? b.line + 1 : b.line, ref: lineElsFor(b.quoted ? b.line + 1 : b.line) }, rows.length ? rows : [h('div', { className: 'dn-code-line', 'data-line': b.line + 1, key: 'e' }, ' ')])
            // A quoted fence keeps the quote bar next to it: inside a `>` passage the code block is
            // part of the quotation, and drawing it flush left would read as a separate paragraph.
            out.push(b.quoted ? h('div', { className: 'dn-quote dn-quote-code', key: key }, [pre]) : pre)
            continue
          }
          if (b.k === 'table') {
            const rows = []
            for (let ri = 0; ri < b.rows.length; ri++) {
              ...
              for (let ci = 0; ci < cells.length; ci++) {
                ...
                const base = at < 0 ? search : at
                search = base + txt.length
                const editing = cellEdit && cellEdit.line === row.line && cellEdit.at === base
                kids.push(h(row.head ? 'th' : 'td', {
                  className: (row.head ? 'dn-th' : 'dn-td') + (editing ? ' dn-cell-edit' : ''),
                  key: 'd' + ci,
                  style: { textAlign: b.align && b.align[ci] ? b.align[ci] : 'left' },
                  // Where this cell's text starts in the SOURCE line, so a click becomes "edit this
                  // cell" without re-parsing the row.
                  ...
                  ? [h('span', { className: 'dn-cell-mirror', key: 'm' }, lineSpans(txt, base, false, row.line)),
                    h('input', {
                      ...
                      onKeyDown: function (ev) {
                        if (ev.key === 'Escape') { ev.preventDefault(); cancelCellEdit() }
                        if (ev.key === 'Enter') { ev.preventDefault(); commitCellEdit() }
                      },
                    })]
      ...
      // The table-cell editor has to be part of the key too: it is the only thing that changes when a
      // cell is entered, and without it the cached block list kept rendering the plain table (the
      // double click looked like it did nothing).
      const blocksKey = (st ? st.text : '') + '|' + mode + '|' + (editBlock ? editBlock.from + ':' + editBlock.to : '') + '|' + (cellEdit ? 'c' + cellEdit.line + ':' + cellEdit.at + ':' + cellEdit.len : '') + '|' + styleKeyOf(st)
      ...
      // The settings card. Small on purpose: one switch today, and the place where the next one goes.
      const settingsEl = settingsOpen ? h('div', { className: 'dn-modal', key: 'settings' }, [
        h('div', { className: 'dn-modal-box', key: 'box' }, [
          h('div', { className: 'dn-modal-title', key: 't' }, '插件设置'),
          h('label', { className: 'dn-modal-switch', key: 'dbl' }, [
            h('input', {
              type: 'checkbox', key: 'c', checked: dblClickOnly,
              onChange: function (e) { setEditGesture(e.target.checked) },
            }),
            h('span', { key: 'l' }, '双击才编辑'),
          ]),
          h('div', { className: 'dn-modal-hint', key: 'h' },
            '开启（默认）：单击正文只是单击，双击才进入就地编辑 —— 避免误触。\n' +
            '关闭：单击正文即进入编辑（光标落在你点的那个字上）。\n' +
            '两种模式下，单击一段标记文字都仍然是弹出标记功能卡；表格单元格在双击时进入就地编辑，代码块编辑的是块体、围栏不会被删。'),
          h('div', { className: 'dn-modal-hint', key: 'k' }, '这个开关记在这台浏览器的本地存储里（和其他卡片偏好一样）。'),
      ...
      if (menuOpen) {
        ...
        if (menuOpen.kind === 'notes') {
          ...
          // 提交到 git used to sit here as well (same doCommit() as the 提交 button in the action
          // row) — two entries for one action, so the menu keeps only what the row does NOT have.
