    ...
    function NoteCard(props) {
      ...
      function applyState(r) {
        ...
        if (jump && jump !== seenJumpRef.current) {
          ...
          // Remember the line the reader was on when they triggered this jump: back/forward returns
          // there. This branch runs for EVERY jump the host reports — a link into another note and an
          // anchor inside this one — and it is the only place that touches the history entry.
          if (navRef.current.at >= 0) {
            const leaving = navRef.current.list[navRef.current.at]
            if (leaving) leaving.line = viewSavedRef.current.line || 0
          }
      ...
      function navGo(delta) {
        ...
        // The recorded line is read ONLY here: a back/forward jump restores the spot the reader left
        // from. Ordinary opening still uses the live reading position, exactly as before.
        if (entry.line >= 1) {
