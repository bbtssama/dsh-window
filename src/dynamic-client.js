  ...
  apply(ctx) {
    ...
    function NoteCard(props) {
      ...
      function onBodyLinkClick(e) {
        ...
        if (href === '' || !isLocalRef(href)) return
        e.preventDefault()
        e.stopPropagation()
        host.call('openMirrorDoc', { sessionId: sidRef.current, href: href }).then(function (r) {
          if (r && r.ok) { notify(r.existed ? ('已切到《' + r.note + '》') : ('已跟随链接打开《' + r.note + '》')) ; refreshNotes() }
              ...
              onClick: function () {
                ...
                host.call('scanFolder', { sessionId: sidRef.current, dir: dir }).then(function (r) {
                  ...
                  setNoteModal(function (p) {
                    return Object.assign({}, p, { files: (r.files || []).map(function (f) { return { name: f.name, size: f.size, on: true } }), rootId: r.rootId, suggestions: null })
                  })
