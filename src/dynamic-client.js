...
'.dn-sel-list{max-height:46%;overflow:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;border-top:1px solid rgba(0,0,0,.1);padding:8px 12px;font-size:12px;flex:0 0 auto;}',
// A selection row: the text block jumps to that passage, the toggle expands it, and the
// text is rendered as inline Markdown (bold/code/links) instead of raw source.
'.dn-sel-jump{cursor:pointer;border-radius:5px;padding:2px 3px;margin:0 -3px;}',
'.dn-sel-jump:hover{background:rgba(59,111,224,.08);}',
'.dn-sel-open .dn-sel-text{max-height:none;}',
...
// How many characters a long press may take as its unit when the run is CJK (see wordRange).
...
return {
  ...
  apply(ctx) {
    ...
    function NoteCard(props) {
      ...
      // Which selection rows are expanded in the panel (by id).
      ...
      function wordRange(pt) {
        ...
        // CJK has no spaces, so "the run of word characters around the press" can be a whole
        // paragraph — a long press then framed a big block of text and felt uncontrollable
        // ("长按某处文字而不拖动时，会自动框选相当多的文字内容"). Latin text keeps word semantics
        // (an identifier like note_take_new_selections stays whole); a CJK run is capped to a
        // phrase-sized window around the press, which the user can still drag out from either
        // end. 12 characters is about a third of a phone line.
        if (b - a > CJK_UNIT_MAX && /[\u3040-\u30ff\u4e00-\u9fff]/.test(rawLine.slice(a, b))) {
          const centre = Math.min(Math.max(pt.col, a), b)
          const from = Math.max(a, Math.min(centre - Math.floor(CJK_UNIT_MAX / 2), b - CJK_UNIT_MAX))
          return { from: from, to: from + CJK_UNIT_MAX }
        }
        return { from: a, to: b }
      }
      function copyText(t) {
      ...
      /** Inline Markdown for a selection row (bold/code/links), one line at a time. */
      ...
      /** Scroll the note to the passage a selection came from (its first line). */
      function jumpToSelection(s) {
        const line = Math.max(1, Math.round(Number(s.startLine) || 1))
        const anchor = String((String(textRef.current || '').split(String.fromCharCode(10))[line - 1] || '')).trim().slice(0, 40)
        pendingViewRef.current = { line: line, anchor: anchor, tries: 0 }
        bump()
      }
      function doReload() {
      ...
      function finishEdit() {
        ...
        if (ta) {
          const line = Math.max(1, Math.round(ta.scrollTop / editLineHeight(ta)) + 1)
      ...
      React.useEffect(function () {
        ...
        const place = function () {
          ...
          try {
            ta.focus()
            ta.setSelectionRange(off, off)
            // Measured line height, and the target line lands near the top of the editor so
            // the reader keeps their place instead of being dropped at line 1.
            ta.scrollTop = Math.max(0, (pt.line - 1) * editLineHeight(ta) - 24)
          } catch (err) { }
        }
      ...
      function renderTokens(toks, keyPrefix) {
        ...
        for (let i = 0; i < toks.length; i++) {
          ...
          if (tk.k === 'link') {
            ...
            const kids = tk.kids && tk.kids.length ? renderTokens(withBase(tk.kids, tk.base || 0), keyPrefix + 'lk' + i + '_') : tk.t
      ...
      const panelEl = panel ? h('div', { className: 'dn-sel-list', key: 'sel' }, selList.length
        ? selList.map(function (s) {
          const open = selOpen[s.id] === true
          return h('div', { className: 'dn-sel-item' + (open ? ' dn-sel-open' : ''), key: s.id }, [
            h('div', { className: 'dn-sel-meta', key: 'm' }, [
              ...
              h('button', {
                className: 'dn-x', key: 'exp', style: { color: '#6b7280' },
                title: open ? '收起这段文字' : '展开这段文字',
                onClick: function () { setSelOpen(function (prev) { const n = Object.assign({}, prev); if (n[s.id]) delete n[s.id]; else n[s.id] = true; return n }) },
              }, open ? '收起' : '展开'),
              h('button', {
            ...
            // The text itself is the jump target: clicking a selection scrolls the note to the
            // line it came from. Links inside are neutralised so a click never navigates away.
            h('div', {
              className: 'dn-sel-jump', key: 'j',
              title: '跳到正文第 ' + s.startLine + ' 行',
              onClick: function (e) { e.preventDefault(); e.stopPropagation(); jumpToSelection(s) },
            }, [
              h('div', { className: 'dn-sel-text', key: 't' }, mdInline(s.text)),
              h('div', { className: 'dn-sel-jump-hint', key: 'h', style: { fontSize: '10.5px', color: '#3b6fe0' } }, open ? '' : '点击跳到正文 · 点[展开]看全段'),
            ]),
            s.remark ? h('div', { className: 'dn-sel-remark', key: 'r' }, ['备注：', mdInline(s.remark)]) : null,
          ])
        })
