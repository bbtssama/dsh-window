...
return {
  apply(ctx) {
    ...
    /**
     * Every `<...>/note/_assets` directory inside a tree — the plugin's own asset area.
     *
    ...
    async function findAssetDirs(dir) {
      ...
      while (stack.length && seen < 50000) {
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          seen += 1
          const full = cur + '/' + name
          const parent = cur.split('/').pop()
          if (name === ASSETS_DIR && parent === NOTES_DIR) { out.push(full); continue }
          stack.push(full)
        }
      }
      return out
    ...
    async function mirrorFolder(srcDir, rootId, opts) {
      ...
      const nested = await findAssetDirs(src)
      if (isWin) {
      ...
      for (let i = 0; i < MIRROR_SKIP_FILES.length; i++) {
        ...
        if (i === 0) {
          const junk = await findAssetDirs(dest)
    ...
    async function listMarkdownIn(dir) {
      ...
      try {
        ...
        // Say what the reader probably MEANT: the closest existing ancestor's directories, with
        // the ones containing what was typed first. "linux学习" →
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        const isDir = e && (e.kind === 'directory' || e.type === 'directory' || e.isDirectory === true)
        if (isDir) continue
        out.push({ name: name, size: await entrySize(e, base + '/' + name) })
      }
      out.sort(function (a, b) { return a.name.localeCompare(b.name) })
      return { ok: true, files: out, rootId: assetRootIdFor(dir) }
    }
    /**
    ...
    async function createNoteFromText(nameRaw, text, meta) {
      ...
      if (canWrite()) {
        ...
        try { await commit('note: 从文件夹导入 ' + name) } catch (err) { }
      }
    ...
    handleLocked('asset', async function (args) {
      ...
      try {
        ...
        const relSegs = String(p).replace(/\\/g, '/').split('/').reduce(function (acc, seg) {
          if (seg === '' || seg === '.') return acc
          if (seg === '..') { acc.pop(); return acc }
          acc.push(seg)
          return acc
        ...
        const norm = safe.split('/').reduce(function (acc, seg) {
          if (seg === '' || seg === '.') return acc
          if (seg === '..') { acc.pop(); return acc }
          acc.push(seg)
          return acc
    ...
    registerToolLocked(harness.defineTool({
      ...
      description: '把一个**文件夹**导入成本会话的笔记：整个目录会被镜像进 note/_assets/<素材根>/（保持原有相对结构），然后在其中选中的 .md 里，每个文件建一份笔记（各自一个 git 仓库）。同一目录里的多个 .md **共享同一份镜像**，不会各复制一份。镜像后的图片按相对路径直接可用（`![](./img/a.png)`）。',
      parameters: {
        dir: { type: 'string', required: true, description: '要导入的文件夹（绝对路径）。' },
        files: { type: 'array', required: true, description: '要建笔记的 .md 文件名（相对该目录，可含子目录）。', items: { type: 'string' } },
      ...
      output: {
        ...
        render: function (a, v) { return [{ type: 'text', text: v.ok ? ('已镜像 ' + v.files + ' 个文件 / ' + Math.round(v.bytes / 1024) + ' KB 到 ' + v.assetRoot + (v.reused ? '（复用已有素材根）' : '') + '\n\n' + v.created) : ('导入失败: ' + v.error) }] },
      },
      async execute(args, exec) {
        return await withNoteLock(noteLockKey(), async function () {
