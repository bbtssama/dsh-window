...
return {
  apply(ctx) {
    ...
    harness.handle('addSelection', async function (args) { return await addSelection(args || {}) })
    ...
    harness.handle('commit', async function (args) { return await commit(args && args.message ? args.message : '') })
    ...
    harness.registerTool(ctx, harness.defineTool({
      ...
      async execute(args, exec) {
        const moved = adoptFromExec(exec); await ensureLoaded(); if (moved) await migrate(); markTouched()
