...
return {
  apply(ctx) {
    ...
    registerToolLocked(harness.defineTool({
      name: 'note_goto',
      description: '让卡片跳到笔记的某一行（和用户自己滚动到那一样，只是由 agent 发起）。可选 note 参数先切到那份笔记。用于"看这里"、"我把改好的地方给你翻出来"。',
      parameters: {
        line: { type: 'integer', required: true, description: '要跳到的行号（1 基）' },
        note: { type: 'string', description: '先切到这份笔记（省略则用当前打开的）' },
        anchor: { type: 'string', description: '那一行的文本片段（省略则自动取该行开头，用于原文变动后重新定位）' },
      },
      output: {
        schema: { type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean', required: true }, line: { type: 'integer', required: true }, lineCount: { type: 'integer', required: true }, active: { type: 'string', required: true }, anchor: { type: 'string', required: true }, error: { type: 'string', required: true } } },
        render: function (a, v) { return [{ type: 'text', text: v.ok ? ('已把《' + v.active + '》翻到第 ' + v.line + ' 行（共 ' + v.lineCount + ' 行）') : ('跳转失败: ' + v.error) }] },
      },
      async execute(args, exec) {
        ...
        return await withNoteLock(noteLockKey(), async function () {
          ...
          if (a.note) {
            ...
            if (!r.ok) return { ok: false, line: 0, lineCount: 0, active: activeNote, anchor: '', error: r.error }
          }
          if (!activeNote) return { ok: false, line: 0, lineCount: 0, active: '', anchor: '', error: '本会话还没有打开任何笔记' }
          const all = linesOf(S.text)
          const line = Math.min(Math.max(1, intOr(a.line, 1)), all.length)
          ...
          return { ok: r.ok === true, line: line, lineCount: all.length, active: activeNote, anchor: anchor, error: r.ok ? '' : String(r.error || '') }
        })
      },
    }))
    registerToolLocked(harness.defineTool({
      name: 'note_panel',
      description: '唤起或收起笔记卡片本身（等价于用户执行 /window-note start / stop）。start 在没有笔记的会话里也能让卡片出现，用户就能点[新建]；不会创建或删除任何笔记。',
      ...
      async execute(args, exec) {
        ...
        return await withNoteLock(noteLockKey(), async function () {
          const a = args || {}
          const act = a.action === 'stop' ? 'stop' : (a.action === 'start' ? 'start' : '')
          if (!act) return { ok: false, action: String(a.action || ''), summoned: summoned === true, notes: (await listNoteDirs()).length, active: activeNote, error: 'action 必须是 start 或 stop' }
          summoned = act === 'start'
          await writeSessionState()
          return { ok: true, action: act, summoned: summoned, notes: (await listNoteDirs()).length, active: activeNote, error: '' }
        })
      },
    }))
    registerToolLocked(harness.defineTool({
