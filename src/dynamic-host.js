...
return {
  apply(ctx) {
    ...
    registerToolLocked(harness.defineTool({
      ...
      description: '领取用户自上次领取之后新划选的内容（默认只取 user 划的，并立即标记为已取用）。回答用户问题前先调用它。返回默认 brief；要某几条的备注与原文用 note_get_selections({ids, detail:"full"})。想再看一遍已取用过的（例如上下文被压缩后重新对齐）传 redeliver:true。',
      parameters: {
